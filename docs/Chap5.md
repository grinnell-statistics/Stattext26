...
\caption{Segmented bar graph of nucleus shape and malignancy.}(\#fig:fig1)
\end{figure}
...
\begin{table}[H]
\centering
\caption{(\#tab:tab1)The numbers of benign and malignant tumors for round and concave cell nuclei.}
\centering
...
In Table 5.1, both the variables are categorical. A **categorical variable** is one for which the measurement
...
Bar graphs are often useful in comparing two categorical variables. Figure 5.1 shows a **segmented bar graph** (also called a stacked bar graph) for the cancer cell data. This graph shows the conditional percentages
for each nucleus shape. About 80% of the concave nuclei are malignant, whereas about 45% of the round
nuclei are malignant.

![(\#fig:fig2)Segmented bar graph of nucleus shape and malignancy.](Chap5_files/figure-latex/fig2-1.pdf) 
...
\caption{(\#tab:tab2)10,000 simulated trials based on Question 8. The $p$–value is $P(X\ge17)=0.022=(2+21+197)/10,000$, where $X$ is the number of concave malignant cells.}
\centering
...
\begin{table}[H]
\centering
\caption{(\#tab:tab3)The numbers of benign and malignant tumors for round and concave cell nuclei.}
\centering
\begin{tabular}[t]{lrrr}
\toprule
\multicolumn{1}{c}{ } & \multicolumn{2}{c}{Type} & \multicolumn{1}{c}{ } \\
...
\caption{(\#tab:tab4)Table of expected counts for the cancer cell study.}
\centering
\begin{tabular}[t]{lrrr}
\toprule
Shape & Benign & Malignant & Total\\
...
weighted by the expected count. The *observed count* represents the observed cell count from Table 5.1, and
the *expected count* represents the expected cell count from Table 5.4.
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab5) Data from Richard Doll’s case‐control study on smoking and lung cancer.}
\centering
\begin{tabular}[t]{lrrr}
\toprule
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab6) Sampling designs and appropriate hypothesis tests.}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab7)Observed results of 30 rolls of a die.}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab8) Observed and expected results of 30 rolls of a die.}
\centering
\begin{tabular}[t]{llrrrrr}
\toprule
  & 1 & 2 & 3 & 4 & 5 & 6\\
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab9)Results from the statistics enrollment study.}
\centering
\begin{tabular}[t]{llr}
\toprule
  & Yes & No\\
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab10)Results of the hypothetical tea‐tasting study.}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab11)Survival status of Donner party members.}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab12)Results from a study of the effect of treatment with different antidepressant drugs (or a placebo) on the success of cocaine addicts in remaining drug-free for at least 3 weeks during a 6-week treatment period.}
\centering
\begin{tabular}[t]{llcc}
\toprule
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab13) Results from the collision study.}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab14) Results of the Zocor study.}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab15) Impact of third generation contraceptive pills on venous thrombosis.}
\centering
