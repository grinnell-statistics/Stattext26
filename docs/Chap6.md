...
\caption{Picture of the space shuttle Challenger just after ignition. Each solid rocket booster had six O-rings, two at each field joint. The O-rings at the right aft field joint failed.}(\#fig:fig1)
\end{figure}
...
\caption{Diargam of a solid rocket booster.}(\#fig:fig2)
\end{figure}
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab1)O‑ring damage on 24 space shuttle launches.}
\centering
...
![(\#fig:fig3)Space shuttle data with a simple linear regression model and a logistic regression model.](Chap6_files/figure-latex/fig3-1.pdf) 
...
![(\#fig:fig4)A scatterplot of the residuals from the space shuttle logistic regression model and a sample of what a scatterplot of normally distributed residuals might look like.](Chap6_files/figure-latex/fig4-1.pdf) 
...
to reject the null hypothesis when it should be rejected.^[?Recall that Z in Equation (6.13) is a statistic calculated from the sample data and Z* in Equation (6.11) is called a critical value. Z* represents a value based on a desired level of confidence. Z* is known before the data are collected, while Z is based on the sample data]
...
>13. The coefficients in Equation (6.9) were calculated when a successful launch was given a value of 1. Con-
...
![(\#fig:fig7) A scatterplot of the observed data and estimated probabilities for both round cells (Concave= 0) and concave cells (Concave= 1).](Chap6_files/figure-latex/fig7-1.pdf) 
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab2)Table 6.2 The Bernoulli model.}
\centering
\begin{tabular}[t]{lcc}
\toprule
Value of y & 0 & 1\\
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab3)Cancer cell data classified into groups based on the size of the radius.}
\centering
...
![(\#fig:fig9)A logistic regression model, with $\hat \pi_i$ estimated from Equation (6.8) using maximum likelihood estimates, plotted with the observed probability of malignancy for the grouped data in Table 6.3.](Chap6_files/figure-latex/fig9-1.pdf) 
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab4)Observed and expected values using the logistic regression model for the Cancercells data, where $\hat\pi_i$ is estimated from the logistic regression model $\hat\pi_i = e^{b_0 + b_1 x_i}/(1 + e^{b_0 + b_1 x_i})$.}
\centering
...
![(\#fig:fig11)Scatterplots of delta deviance, delta chi-square, and delta beta values versus the expected probabilities from the space shuttle data. Circled values represent launches at 75°F.](Chap6_files/figure-latex/fig11-1.pdf) 
...
![(\#fig:fig12) Scatterplots of delta chi-square and delta beta values versus leverage from the space shuttle data. The extreme value along the y-axis represents the launches at 75°F. There do not appear to be any extreme leverage values.](Chap6_files/figure-latex/fig12-1.pdf) 
