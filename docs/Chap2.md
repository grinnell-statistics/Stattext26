...
\caption{Individual value plot of the data from the color distracter and standard games.}(\#fig:fig2)
\end{figure}
...
\caption{Histogram of residuals and plot of residuals versus color.}(\#fig:fig3)
\end{figure}
...
\caption{Dotplots representing data from two studies. The difference between the group means is the same in both data sets, but the random variation is not the same. The variability in the residuals is much larger for 'Results A' than for 'Results B'.}(\#fig:fig4)
\end{figure}
...
\caption{Two histograms of the computer game study residuals.}(\#fig:fig5)
\end{figure}
...
\caption{Bulge rule showing appropriate transformations to linearize curved data.}(\#fig:fig6)
\end{figure}
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab2)One-factor ("one-way") ANOVA table (one factor with $I$ levels).}
\centering
...
\begin{table}[!h]
\centering
\caption{(\#tab:tab3) Plots that can be used to evaluate model assumptions about the residuals.}
\centering
\begin{tabular}[t]{ll}
\toprule
Assumption & Plot\\
