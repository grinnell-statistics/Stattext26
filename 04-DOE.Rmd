...
13. Use the technology instructions provided on the CD to check your answers. Submit the software output. Note that a $\scriptstyle p$-value for each $\scriptstyle F$-statistic is provided. State your conclusions about each of the three hypotheses based on these $\scriptstyle p$-values.  
...
18. Use a statistical software package to simultaneously test all six hypotheses given at the beginning of this section.
 ...
 (c) Use the $\scriptstyle p$-value corresponding to each hypothesis to state your conclusions.
 (d) Address how random sampling and random allocation influence your conclusions.

19. Determine whether $\scriptstyle MS_{\text{Brand}}$, $\scriptstyle MS_{\text{Time}}$, and $\scriptstyle MS_{\text{BrandTime}}$ are the same as in Question 13. Explain why the $\scriptstyle F$-statistics corresponding to these three hypotheses have changed.
...
31. Table 4.6 is a table with two rows and three columns, similar to the interaction effect term in the two-way factorial diagram in Figure 4.4. However, for this question we will assume that only two effects are known: $\scriptstyle \widehat{(\alpha\beta)}_{11} = 2$ and $\scriptstyle \widehat{(\alpha\beta)}_{12}$ = -5.
...
```
 ...
 (a) Equation \ref{4.8} states that all the $\scriptstyle AB$ effects within Brand C add up to zero [$\scriptstyle \widehat{(\alpha\beta)}_{11} \ +\  \widehat{(\alpha\beta)}_{12} \ +\  (\widehat{(\alpha\beta)}_{13}$ = 0]. Use this rule to calculate $\scriptstyle \widehat{(\alpha\beta)}_{13}$.

 (b) Equation \ref{4.8} also states that all the $\scriptstyle AB$ effects within 0 water amount add up to zero (the same for 5 and 15 drops of water). Use this rule to calculate $\scriptstyle \widehat{(\alpha\beta)}_{21}$, $\scriptstyle \widehat{(\alpha\beta)}_{22}$, and $\scriptstyle \widehat{(\alpha\beta)}_{23}$.

 (c) Consider a different interaction table with two rows and three columns. Explain why it is not possible to have effects of $\scriptstyle \widehat{(\alpha\beta)}_{11} = 4$, $\scriptstyle \widehat{(\alpha\beta)}_{13} = -4$, and $\scriptstyle \widehat{(\alpha\beta)}_{22}$ = 6 and still follow the restrictions in Equation 4.8.
...
32. Table 4.7 is another table of interaction effects, with five rows and three columns (five levels of factor $\scriptstyle A$ and three levels of factor $\scriptstyle B$). Again, we will assume that only some of the effects are known.
...
33. Use the previous two questions to determine the degrees of freedom for an interaction term for a balanced completely randomized design using three levels of factor $\scriptstyle A$ and four levels of factor $\scriptstyle B$.
...
34. **Checking Assumptions** In the statistical model in Equation \ref{4.4}, the following assumptions need to be validated about the random error terms, $\scriptstyle \varepsilon_{ijk}$, before any formal hypothesis test can be developed:
...
Note that the second assumption includes an equal variance assumption about the random errors from the different factor-level groups: $\scriptstyle \sigma_{11}^2 \ =\  \sigma_{12}^2 \ =\ \sigma_{13}^2 \ =\  \sigma_{21}^2 \ =\  \sigma_{22}^2 \ =\  \sigma_{23}^2$.
 ...
 (c) Transform the response variable using natural log ($\scriptstyle \log(\text{Strength})$) and $\scriptstyle \sqrt{\text{Strength}}$. Did both transformations improve the equal variance assumption?
...
41. Compare $\scriptstyle MS_{C2}$ and $MS_{C3}$, the mean squares found in Question 38.
