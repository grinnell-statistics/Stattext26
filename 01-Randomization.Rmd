...
## \textcolor{blue}{Investigation: Can a New Drug Reduce the Spread of Schistosomiasis?}
...
```{r graph1, warning=FALSE, echo=FALSE, message=FALSE, fig.height = 2, fig.width= 3, fig.cap="Individual value plot of the worm count data"}
...
## \textcolor{blue}{Statistical Inference Through a Randomization Test}
...
## \textcolor{blue}{ Performing a Randomization Test Using a Computer Simulation}
...
### \textcolor{blue}{Practice:} Using Computer Simulations to Conduct a Hypothesis Test{-}
...
9. Use the instructions to write a program, function, or macro to repeat the process 10,000 times. Count the number of simulations where the difference between the group averages (control minus K11777) is greater than or equal to 7.6, divide that count by 10,000, and report the resulting empirical $\scriptstyle p$-value.
...
```{r graph2, echo=FALSE, out.width="70%", fig.align="center",fig.cap =  "Histogram showing the results of a schistosomiasis simulation study. In this simulation, 281 out of 10,000 resulted in a difference greater than or equal to 7.6.."}
knitr::include_graphics("docs/Fig1_2.png")
```
...
### \textcolor{blue}{Practice:} A Two-Sided Hypothesis Test{-}

13. Run the simulation study again to find the empirical $\scriptstyle p$-value for a two-sided hypothesis test to determine if there is a difference between the treatment and control group means for female mice.
...
15. Explain why you expect the $\scriptstyle p$-value for the two-sided alternative to be about double that for the onesided alternative. Hint: You may want to look at Figure 1.2

16. Using the two-sided alternative hypothesis, the two-sample t-test provides a $\scriptstyle p$-value of 0.022.^[When we do not assume equal variances Minitab uses 7 degrees of freedom providing a $\scriptstyle p$-value of 0.022 while R uses 7.929 degrees of freedom resulting in a $\scriptstyle p$-value of 0.0194.] This $\scriptstyle p$-value would provide strong evidence for rejecting the assumption that there is no difference between the treatment and the control (null hypothesis). However, this $\scriptstyle p$-value should not be used to draw conclusions about this study. Explain why.
...
## \textcolor{blue}{What Can We Conclude from the Schistosomiasis Study?}
...
## \textcolor{blue}{Permutation Tests versus Randomization Tests}
...
### \textcolor{blue}{Practice:} Is There Evidence of Age Discrimination? {-}
\vspace{-1em}
Data set: \texttt{Age}
...
18. Modify the program/macro you created in Question 17 to conduct a one-sided hypothesis test to determine if the median age of people who were laid off is higher than the median age of people who were not laid off. Report the $\scriptstyle p$-value and compare your results to those in Question 17.
...
## \textcolor{blue}{Permutation and Randomization Tests for Matched Pairs Designs}
...
### \textcolor{blue}{Practice:} Testing the Effect of Music on Relaxation{-}
\vspace{-1em}
Data set: `Music`
...
## \textcolor{blue}{The Bootstrap Distribution}
...
### \textcolor{blue}{Practice:} Creating a Sampling Distribution and a Bootstrap Distribution{-}
\vspace{-1em}
Data set: `ChiSq`
...
## \textcolor{blue}{Using Bootstrap Methods to Create Confidence Intervals}
...
### \textcolor{blue}{Practice:}Estimating Salaries of Medical Faculty{-}
\vspace{-1em}
...
## \textcolor{blue}{Relationship Between the Randomization Test and the Two-Sample t-Test}
...
## \textcolor{blue}{Wilcoxon Rank Sum Tests for Two Independent Samples}
...
### \textcolor{blue}{Practice:} *Wilcoxon Rank Sum Tests*{-}
\vspace{-1em}
...
## \textcolor{blue}{Kruskal-Wallis Test for Two or More Independent Samples}
...
### \textcolor{blue}{Practice:} Kruskal-Wallis Test{-}
\vspace{-1em}
Data set: `NLBB Salaries`
...
## \textcolor{blue}{Multiple Comparisons}
...
### \textcolor{blue}{Practice:} Comparing Car Prices{-}
\vspace{-1em}
Data set: `Car1`
...
### \textcolor{blue}{Practice:} The Least-Significant Differences Method and the Bonferroni Method{-}
\vspace{-1em}
Data set: `Car1`

When the significance level is controlled for each individual test, as was done in Question 29, the process is often called the **least-significant differences method (LSD)**. Notice that using $\scriptstyle \alpha$ = 0.05 for all tests has some undesirable properties, especially when a large number of tests is being conducted. If 100 independent tests were conducted to compare multiple groups (and there really were no differences), the probability of incorrectly rejecting at least one test would be 1 - 0.95$^{100}$ = 0.994. Thus, using $\scriptstyle \alpha$= 0.05 as a critical value for 100 comparisons will almost always lead us to incorrectly conclude that some results are significantly different.
...
>**MATHEMATICAL NOTE:** Other terms that are commonly discussed with multiple comparisons are **family-wise type I error** and **comparison-wise type I error**. Bonferroni’s method is an example of a technique that maintains the family-wise type I error. With the family-wise type I error 0.05, assuming that there really is no difference between any of the K pairs, there is only a 5% chance that any test will reject $\scriptstyle H_0$. The least-significant differences method is used to maintain a comparison-wise type I error rate: Assuming that a particular null hypothesis test is true, there is a 5% chance we will (incorrectly) reject that particular hypothesis. Montgomery’s *Design and Analysis of Experiments* text provides more information on multiple comparisons.$^{11}$
