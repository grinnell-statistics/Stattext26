...
>**MATHMATICAL NOTE**  For any population with mean $\mu$ and finite standard deviation $\sigma$, the **central limit theorem** states that the sample mean $\bar{x}$ from an independent and identically distributed sample tends to follow the normal distribution if the sample size is large enough. The mean of \(\small \bar{x}\) is the same as the population mean, \(\small \mu\), while the standard deviation of x is \(\small \sigma / \sqrt{n}\), where {\small $n$} is the sample size.
