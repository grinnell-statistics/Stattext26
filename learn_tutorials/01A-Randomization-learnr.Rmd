...
# The three data sets below are the chapter's Mice, Age, and Music data sets,
# entered here exactly as they appear in the STA310data files so that the
# tutorial runs without downloading anything. Data set names and variable
# names match those used in 01-Randomization.Rmd.

# --- Mice: schistosomiasis worm-count data ----------------------------------
Mice <- data.frame(
  Female.Trt = c(1, 2, 2, 10, 7),     # mean  4.4
  Female.Ctl = c(16, 10, 10, 7, 17),  # mean 12.0
  Male.Trt   = c(3, 5, 9, 10, 6),
  Male.Ctl   = c(31, 26, 28, 13, 47)
)

# --- Age: Westvaco age-discrimination layoff data ---------------------------
Age <- data.frame(
  age    = c(25, 33, 35, 38, 48, 55, 55, 55, 56, 64),
  layoff = c("no", "no", "no", "no", "no", "no", "yes", "yes", "no", "yes")
)

# --- Music: pulse-rate matched-pairs data -----------------------------------
Music <- data.frame(
  Prepulse                = c(72, 66, 70, 58, 65, 62, 72, 54, 69, 59, 63, 63, 60, 55, 56, 70, 75, 45, 72, 66, 62, 76, 56, 61, 79, 72, 70, 56),
  Slowsong                = c(78, 66, 86, 63, 80, 61, 76, 59, 71, 59, 69, 59, 57, 58, 60, 75, 77, 52, 71, 67, 62, 75, 62, 59, 81, 72, 68, 52),
  Fastsong                = c(80, 69, 78, 59, 74, 63, 77, 61, 72, 61, 78, 62, 63, 66, 63, 75, 79, 48, 73, 65, 63, 77, 65, 61, 94, 78, 69, 54),
  Favmusic                = c(1, 3, 3, 2, 3, 2, 2, 3, 4, 3, 1, 3, 4, 3, 4, 3, 3, 4, 3, 3, 3, 3, 2, 1, 3, 2, 2, 3),
  Slowdiff                = c(6, 0, 16, 5, 15, -1, 4, 5, 2, 0, 6, -4, -3, 3, 4, 5, 2, 7, -1, 1, 0, -1, 6, -2, 2, 0, -2, -4),
  Fastdiff                = c(8, 3, 8, 1, 9, 1, 5, 7, 3, 2, 15, -1, 3, 11, 7, 5, 4, 3, 1, -1, 1, 1, 9, 0, 15, 6, -1, -2),
  Fastdiff_Minus_Slowdiff = c(2, 3, -8, -4, -6, 2, 1, 2, 1, 2, 9, 3, 6, 8, 3, 0, 2, -4, 2, -2, 1, 2, 3, 2, 13, 6, 1, 2)
)

# Long form of the Mice data, used for the table and plot below.
Mice_long <- Mice %>%
  pivot_longer(cols = everything(), names_to = "group", values_to = "Number") %>%
...
> **TUTORIAL NOTE:** The chapter's `Mice`, `Age`, and `Music` data sets are already loaded for you — there are no external files to download, so every exercise runs entirely inside this sandbox. The data set names and the variable names are the same ones used in the chapter, and each practice question begins with a `head()` of the relevant data frame so you can see what you are working with.
...
```{r table1, echo=FALSE}
Mice %>%
  bind_rows(summarize(Mice, across(everything(), mean))) %>%
  mutate(Mouse = c(as.character(1:nrow(Mice)), "Mean"), .before = 1) %>%
...
ggplot(Mice_long, aes(x = Gender, y = Number, fill = Type, shape = Type)) +
  geom_dotplot(
...
The `Mice` data set stores one column per sex-by-group combination:

```{r q1_head, echo=TRUE}
head(Mice)
```

Recreate Figure 1.1 below, then answer Question 1 from the plot. Because the plot needs one row per mouse, the code first reshapes `Mice` into long form and splits the column names (for example, `Female.Trt`) into a `Gender` and a `Type` column.
...
```{r sec1_ex2-hint}
# After the reshape, the columns are Gender, Type, and Number.
```
...
Mice %>%
  pivot_longer(cols = everything(), names_to = "group", values_to = "Number") %>%
  separate(group, into = c("Gender", "Type"), sep = "\\.") %>%
  ggplot(aes(x = Gender, y = Number, color = Type)) +
  geom_jitter(width = 0.1, height = 0, size = 3) +
  labs(x = "Sex", y = "Worm count", color = "Group") +
  theme_minimal()
```
...
```{r sec1_ex1-hint}
# group_by() takes the two grouping columns, Gender and Type; summarize()
# takes one expression per summary statistic, e.g. mean(Number), sd(Number), ...
```
...
Mice %>%
  ...
  summarize(
    mean_worms   = mean(Number),
    sd_worms     = sd(Number),
    median_worms = median(Number),
    min_worms    = min(Number),
    max_worms    = max(Number),
    .groups = "drop"
  )
```
...
female_worms <- c(Mice$Female.Trt, Mice$Female.Ctl)
...
```{r q17_head, echo=TRUE}
head(Age)
```

```{r sec5_ex1, exercise = TRUE}
observed_diff <- mean(Age$age[Age$layoff == "yes"]) -
                 mean(Age$age[Age$layoff == "no"])

ages <- Age$age

set.seed(7)
null_diffs <- replicate(10000, {
  shuffled_labels <- ___(Age$layoff)
  mean(ages[shuffled_labels == "yes"]) - mean(ages[shuffled_labels == "no"])
})
...
```{r sec5_ex1-hint}
# Shuffle the *labels*, not the ages: sample(Age$layoff) keeps the same
# three "yes" and seven "no" values, just reassigned at random.
```
...
set.seed(7)
null_diffs <- replicate(10000, {
  shuffled_labels <- sample(Age$layoff)
  mean(ages[shuffled_labels == "yes"]) - mean(ages[shuffled_labels == "no"])
})
...
```{r q20_head, echo=TRUE}
head(Music)
```

```{r sec6_ex1, exercise = TRUE}
observed_mean_diff <- mean(Music$Fastdiff_Minus_Slowdiff)

set.seed(11)
null_means <- replicate(10000, {
  signs <- sample(c(-1, 1), size = nrow(Music), replace = TRUE)
  mean(Music$Fastdiff_Minus_Slowdiff * ___)
})
...
```{r sec6_ex1-hint}
# Multiply each subject's difference by its randomly chosen sign:
# Music$Fastdiff_Minus_Slowdiff * signs
```
...
null_means <- replicate(10000, {
  signs <- sample(c(-1, 1), size = nrow(Music), replace = TRUE)
  mean(Music$Fastdiff_Minus_Slowdiff * signs)
})
