...
## \textcolor{blue}{ Investigation: Do Distracting Colors Influence the Time to Complete a Game?}

In 1935, John Stroop published a paper presenting his research on the reaction time of undergraduate students identifying ink colors.$^2$ He found that students took a longer time identifying ink colors when the ink was used to spell a different color. For example, if the word "\textcolor{blue}{yellow}" was printed in blue ink, students took longer to identify the blue ink because they automatically read the word “yellow.” Even though students were told only to identify the ink color, the automatized behavior of reading interfered with the task and slowed their reaction time.^[Many psychologists would call this procedural knowledge instead of automatized behavior. Both are processes that can be done without conscious thought, but automatized behaviors are processes that cannot be slowed down, do not
...
```{r fig21, echo=FALSE, out.width="100%", fig.align="center", fig.cap="An image of the electronic Shapesplosion game with and without color distracters. The instructions for the game were to click and drag each peg to the space with the matching shape."}
...
### \textcolor{blue}{Practice:} Understanding the Study Design {-} 
...
## \textcolor{blue}{ The Two-Sample t-Test to Compare Population Means}
...
variance $\scriptstyle \sigma^2$ . This is called the **equal variance assumption**. Some introductory statistics courses discuss only a two-sample t-test that does not require the equal variance assumption. The equal variance assumption is
...
### \textcolor{blue}{Practice:} Statistical Models for the Two-Sample t-Test {-} 
...
6. Use the game study and the data in the file Games1 to identify $\scriptstyle n_1$, $\scriptstyle n_2$, $\scriptstyle y_{1,12}$ , $\scriptstyle y_{2,12}$ , $\scriptstyle \epsilon_{1, 12}$, and $\scriptstyle \epsilon_{2, 12}$,
...
### \textcolor{blue}{Practice:} Checking Assumptions for the t-Test{-}
\vspace{-1em}
Data set: `Games1`
...
### \textcolor{blue}{Practice:} Calculating a Regression Model and Hypothesis Test for the Slope {-}
\vspace{-1em}
Data set: `Games1`

11. Use the software instructions and the Games1 data to create indicator variables where $\scriptstyle x = 1$ represents the color distracter game and $\scriptstyle x = 0$ represents the standard game. Develop a regression model using Time as the response and the indicator variable as the explanatory variable.

12. Use statistical software to calculate the t-statistic and $\scriptstyle p$-value for the hypothesis tests $\scriptstyle H_0 : \beta_1 = 0$ versus $\scriptstyle H_1 : \beta_1 \ne 0$. In addition, construct a 95% confidence interval for $\scriptstyle \beta_1$ . Based on these statistics, can you conclude that the coefficient, $\scriptstyle \beta_1$, is significantly different from zero? Details for calculating these statistics by hand are provided in the later activities.

13. Repeat the two previous questions, but use an indicator variable where $\scriptstyle x = 1$ represents the standard game and $\scriptstyle x = 0$ represents the color distracter game. Compare the regression line, hypothesis test, and $\scriptstyle p$-value to those from the previous questions. When there are only two categories (color distracter and standard), does the choice of indicator variable impact your conclusions? Why or why not?
...
### \textcolor{blue}{Practice:} Checking Model Assumptions {-}
\vspace{-1em}
Data set: `Games1`
...
## \textcolor{blue}{ ANOVA to Compare Population Means}
...
18. Why doesn’t $\scriptstyle \mu$ have any subscript in the ANOVA model?
...
>**Note:** Throughout this chapter, $\scriptstyle \bar{y}_{1.} = \bar{y}_{1}$ and $\scriptstyle \bar{y}_{2.} = \bar{y}_{2}$. The dot notation is often used with more complex models to indicate that the average was taken over all values of that subscript. For example, $\scriptstyle bar{y}_{2.}$ averages over all $\scriptstyle j = 1, 2, 3, ... , n_2$, observations from the standard game sample results.
...
### \textcolor{blue}{Practice:} Estimating the Model Values {-}
\vspace{-1em}
Data set: `Games1`

19. Use the `Games1` data to calculate $\scriptstyle \bar{y}_{..}$, $\scriptstyle \bar{y}_{1.}$, and $\scriptstyle \bar{y}_{2.}$.
...
22. Calculate the residual for the 20th observation from the standard group, $\scriptstyle \hat{e}_{2, 20}$.


### Model Assumptions for ANOVA {-}

The model assumptions for ANOVA are equivalent to those for the two previous tests. In fact, the assumptions discussed in this section are called the six **Fisher assumptions**, after Ronald Fisher, who developed the ANOVA and the corresponding $\scriptstyle F$-test.
\vspace{-1em}
\begin{itemize}
...
The following questions provide an opportunity to use software to calculate an **$\scriptstyle F$-statistic** (the test statistic for $\scriptstyle H_0:\  \alpha_1 \ =\  \alpha_2 \ =\  0$ that is calculated using an ANOVA table) and corresponding $\scriptstyle p$-value. In addition, you will use graphs to visualize the residuals to check the model assumptions. The later activities will describe the ANOVA calculations in more detail.

### \textcolor{blue}{Practice:} Checking Assumptions {-}
\vspace{-1em}
Data set: `Games1`

23. Use statistical software to calculate the $\scriptstyle F$-statistic and find the $\scriptstyle p$-value. Use the $\scriptstyle p$-value to draw conclusions from this study.

24. How does the $\scriptstyle p$-value in ANOVA compare to the $\scriptstyle p$-value you found for the two-sample $\scriptstyle t$-test and the regression model?

25. Take the square root of the $\scriptstyle F$-statistic in the ANOVA table. Does this value look familiar? Explain.
...
27. Compare the three statistical models. Describe the connections among the $\scriptstyle t$-test, ANOVA, and regression. Why are the $\scriptstyle p$-values the same for all three models?

## \textcolor{blue}{ Comparing Planned Variability to Random Variability}
...
Figure 2.4 shows dotplots for two fictitious data sets, `Results A` and `Results B`. Notice that the differences between group means are identical. In other words, the numerator of the test statistic (difference between group means) is the same for both data sets.
...
## \textcolor{blue}{ What Can We Conclude from the Game Study?}
...
## \textcolor{blue}{ Normal Probability Plots to Assess Normality}
...
```{r fig25, echo=FALSE, out.width="80%", fig.align="center", fig.cap="Two histograms of the computer game study residuals."}
knitr::include_graphics("docs/Fig2_5.png")
```

### \textcolor{blue}{Practice:} Creating Probability Plots {-}
\vspace{-1em}
Data set: `Normal`

The following questions ask you to work through the process of calculating and interpreting probability plots.

28. **Calculating a Normal Probability Plot by Hand**  Consider the following sample data set of $\scriptstyle n = 5$ observations: 14, 11, 17, 15, 13. Complete the following steps to create a normal probability plot.
\vspace{-1em}
 (a) Sort the data from smallest to largest. Use a subscript in parentheses, $\scriptstyle (i)$, to represent the ordered data. For example $\scriptstyle y_{(1)} = 11$ is the smallest observation and $\scriptstyle y_{(5)} = 17$ is the largest observed value.  
...
29. **Plotting Normal Data**  For this problem, use the Normal data set. The first column of data actually is a random sample from a normal distribution. \vspace{-1em}
...
>**NOTE:** As with any other hypothesis test, when we fail to reject $\scriptstyle H_0$ we do not prove $\scriptstyle H_0$ is true. Normal probability plots cannot be used to prove that the data came from a normal distribution ($\scriptstyle H_0$), but they can be used to show that the data are consistent with data from a normal population.
...
### \textcolor{blue}{Practice:} Understanding Variability in Random Samples {-}
\vspace{-1em}
Data set: `Games1`
...
30. If you have no experience with probability plots, it can be helpful to look at several sample data sets that actually do come from a normal distribution. Use software to draw several random samples of the same size from an actual normal population and create normal probability plots. These plots can be compared to the normal probability plot from the actual data. If the real data plot looks similar to the plots where you know that the population is normal, then your data are consistent with the null hypothesis (i.e., the data came from a normal population). If the real data plot is “extreme” (quite different from the plots coming from a normal population), then the differences are not likely due to chance and you can reject the hypothesis that the data came from a normal population.  
 ...
 (b) Use software to draw a random sample of $\scriptstyle n = 40$ from an actual normal probability distribution with mean 0 and standard deviation 1. Create a normal probability plot of the sample data.  
...
The **square-root transformation** ($\scriptstyle y^{1/2} = \sqrt{y}$) is commonly used when the response variable represents counts, such as a count of the number of observed species. Square-root transformations are also very useful when the variances are proportional to the means.
...
31. The data set Emission provides hydrocarbon emission in parts per million (ppm) at idling speed for cars, based on the year each car was manufactured. These data were randomly sampled from a much larger study on pollution control in Albuquerque, New Mexico.  
 ...
 (a) Create individual value plots or side-by-side boxplots of Emission versus Year. Compare the mean and standard deviation of each group. Do the data within each group look consistent with data from a normal population?  
  ...
  (b) Transform the response by taking the log of Emission. Create individual value plots or side-by-side boxplots of $\scriptstyle \log(\text{Emission})$ versus Year. Compare the plot of the transformed data to the plot in Part A. Which plot shows data that better fit the model assumptions?  

  (c) Calculate an ANOVA table, F-test, and $\scriptstyle p$-value to determine if the average log(Emission) varies based on Year. Note that the end-of-chapter exercises and Section 2.9 show that ANOVA can compare more than two groups. In this question, $\scriptstyle I = 5$ groups instead of 2 groups. However, the model and calculations are identical except that now $\scriptstyle i = 1, 2, 3, 4, 5$ instead of $\scriptstyle i = 1, 2$. The null hypothesis is $\scriptstyle H_0\!:\! \mu_1 = \mu_2 = \mu_3 = \mu_4 = \mu_5$ versus the alternative $\scriptstyle H_a\!:\!$ at least one group mean is different from another.  
...
### \textcolor{blue}{Practice:} Transforming Brain and Body Weight Data {-}
\vspace{-1em}
Data set: `Weight`

32. The Weight data set contains the brain weights ($\scriptstyle y$) and body weights ($\scriptstyle x$) of 32 animal species.  

 (a)  Create a scatterplot of $\scriptstyle y$ versus $\scriptstyle x$ with a regression line ($\scriptstyle \hat{y} = b_0 + b_1x$), a plot of residuals versus the explanatory variable, a plot of residuals versus predicted (or “fitted”) values ($\scriptstyle \hat{y}$), and either a normal probability plot or a histogram of the residuals.  

 (b) Try various transformations of the explanatory and response variables to create a better linear regression model. Hint: Notice that since both the $\scriptstyle x$ and the $\scriptstyle y$ variable are right skewed and have outliers, both may need a transformation.
...
### \textcolor{blue}{Practice:} Comparing Four $\scriptstyle (x, y)$ Data Sets {-}
\vspace{-1em}
Data set: `RegrTrans`
...
\vspace{-1em}
 (a) Create a scatterplot of $\scriptstyle y$ versus $\scriptstyle x$ with a regression line ($\scriptstyle \hat{y} = b_0 + b_1 x$), a plot of residuals versus the explanatory variable, a plot of residuals versus predicted (or “fitted”) values ($\scriptstyle \hat{y}$), and either a normal probability plot or a histogram of the residuals.  
  ...
  (b) By hand, sketch on the scatterplot a curve that would fit the data better than the regression line. Notice that the plot of residuals versus the explanatory variable emphasizes the patterns in the residuals much better than does the scatterplot of $\scriptstyle y$ versus $\scriptstyle x$.  
...
## \textcolor{blue}{ Calculating Test Statistics}
...
### The Two-Sample $\scriptstyle t$-Test with the Equal Variance Assumption {-}
...
The **pooled standard deviation** (denoted $\scriptstyle s_p$) uses a weighted average of the two sample variances in order to estimate the size of the variation in a typical random error term (i.e., $\scriptstyle \sigma$, the common standard deviation for the two populations).
...
### \textcolor{blue}{Practice:} Calculating the Two-Sample t-Test {-}

Data set: `Games1`

34. Use Equation 2.7 to calculate the test statistic ($\scriptstyle t$) by hand (i.e., without statistical software) for the computer game study. Use software or a $\scriptstyle t$-table with $\scriptstyle (n_1 + n_2 - 2)$ degrees of freedom to find the $\scriptstyle p$-value.
...
### \textcolor{blue}{Practice:} Testing the Slope Coefficient {-}
\vspace{-1em}
Data set: `Games1`

35. Without statistical software, use summary statistics and Equation 2.9 to calculate the test statistic under the null hypothesis that $\scriptstyle \beta_1 = 0$. Use software or a $\scriptstyle t$-table with $\scriptstyle n - 2 = n_1 + n_2 - 2$ degrees of freedom to find the $\scriptstyle p$-value.

36. Compare the test statistic and $\scriptstyle p$-values in Questions 34 and 35.
...
**Sums of squares ($\scriptstyle SS$)** are measures of spread, calculated in an ANOVA table like the one you saw in the software output for Questions 23 through 25. The three sums of squares calculated for the ANOVA table for the computer game study are described below.

**Group sum of squares ($\scriptstyle SS_{\text{Group}}$)** measures the difference between group means (also called level means). Group sum of squares represents the variability we want in the model. For the computer game, $SS_{\text{Group}}$ measures the spread between the two game type means, but ANOVA can be extended to more than just two groups.
...
**Error sum of squares ($\scriptstyle SS_{\text{Error}}$)** measures the spread of the observed residuals. Recall that each residual is defined as an observed value minus the estimated mean response: $\scriptstyle \hat{e}_{i,j} = y_{i,j} - \bar{y}_{i.}$. In any ANOVA model with one explanatory variable, the mean response is the level average.
...
**Total sum of squares ($\scriptstyle SS_{\text{Total}}$)** measures the overall spread of the responses in the full data set.
...
**Mean squares** ($\scriptstyle MS$) are measures of “average” spread and are calculated by dividing a sum of squares
(SS) by its associated degrees of freedom (df).

**Group Mean squares ($\scriptstyle MS_{\text{Group}}$)** equals $\scriptstyle SS_{\text{Group}}/df_{\text{Group}}$. $\scriptstyle MS_{\text{Group}}$ is a measure of variability between the levels of each factor and is often called **between-level variability**. It is actually just the variance of the level means.

**Mean square error (MSE)** equals $\scriptstyle SS_{\text{Error}}/df_{\text{Error}}$. MSE is a pooled measure of the variability within each level, or the **within-level variability**. Remember that the variance is assumed to be the same for the responses within each level of the factor: $\scriptstyle \sigma_1^2 = \sigma_2^2 = \sigma^2$. When $\scriptstyle I = 2$, MSE is identical to the pooled variance ($\scriptstyle s_p^2$) used in the two-sample $\scriptstyle t$-statistic in Equation 2.7.
...
The $\scriptstyle F$-statistic ($\scriptstyle MS_{\text{Group}}$/MSE) is a ratio of the between-level variability to the within-level variability. If indeed $\scriptstyle \mu_1 = \mu_2$ (i.e., $\scriptstyle H_0:\  \alpha_1 \ =\  \alpha_2 \ =\  0$ is true), then we would expect the variation between the level means in our sample data to be about the same as the typical variation within levels and the $\scriptstyle F$-statistic would be close to one. Larger values of the $\scriptstyle F$-statistic would imply that the level means were farther apart than chance error alone could explain. These calculations are often summarized in an **ANOVA table**, as shown in Table 2.1.
...
### \textcolor{blue}{Practice:} Calculating an ANOVA Table {-}
\vspace{-1em}
Data set: `Games1`

37. Use $\scriptstyle n_1$, $\scriptstyle n_2$, $\scriptstyle \bar{y}_1$, $\scriptstyle \bar{y}_2$, $\scriptstyle \bar{y}_{..}$, $\scriptstyle s_1$, and $\scriptstyle s_2$ to calculate $\scriptstyle SS_{\text{Type}}$ (i.e., $\scriptstyle SS_{\text{Group}}$), $\scriptstyle SS_{\text{Error}}$, $\scriptstyle MS_{\text{Type}}$, and MSE for the computer game study.   Since the group for the computer game study is game type (the Type variable in the data set), we will use the more descriptive labels $\scriptstyle MS_{\text{Type}}$, $\scriptstyle SS_{\text{Type}}$, and $\scriptstyle df_{\text{Type}}$ instead of $\scriptstyle MS_{\text{Group}}$, $\scriptstyle SS_{\text{Group}}$, and $\scriptstyle df_{\text{Group}}$. This is common practice and is similar to how statistical software reports results based on variable names.

38. Calculate the overall variance of the completion times in the entire data set and use it to find the total sum of squares ($\scriptstyle SS_{\text{Total}}$) for the computer game study. Confirm that $\scriptstyle SS_{\text{Total}} = SS_{\text{Type}} + SS_{\text{Error}}$ for the computer game study.

39. Calculate the $\scriptstyle F$-statistic and use software or an $\scriptstyle F$-distribution table with $\scriptstyle df_{\text{Type}}$ and $\scriptstyle df_{\text{Error}}$ degrees of freedom to find the $\scriptstyle p$-value.

## \textcolor{blue}{ Confidence Intervals}
...
The **standard error of the estimate** is a measure of the variability of the statistic. For example, a 95% confidence interval for $\scriptstyle \mu_1$ is
...
### \textcolor{blue}{Practice:} Calculating a Confidence Interval for the Regression Coefficient {-}

Data set: `Games1`

40. Note that the “standard error of the estimate” in the confidence interval is identical to the denominator in the test statistic for the corresponding hypothesis test. Use this information to *write out the formula* for a 95% confidence interval for $\scriptstyle \beta_1$. Use the output from the corresponding $\scriptstyle t$-test to find the estimate, the critical value, and the standard error of the regression coefficient. Use this information to calculate a 95% confidence interval for $\scriptstyle \beta_1$.

>**MATHEMATICAL NOTE:**  The $\scriptstyle F$-distribution used in ANOVA is not a symmetric distribution and all values are positive. Confidence intervals for the difference between two means are not calculated with an $\scriptstyle F$-distribution. Note that the MSE in ANOVA $\scriptstyle = s_p$ from the two-sample $\scriptstyle t$-test and the square root of the critical value $\scriptstyle \sqrt{F_{1,38}} = t^*_{38}$. [[[[]]]]
...
Throughout this chapter, you were asked to evaluate **residual plots** to determine whether the model assumptions were met. When model assumptions are not met, the test statistics do not follow the corresponding $\scriptstyle t$-distribution or $\scriptstyle F$-distribution. Thus, the $\scriptstyle p$-values may not be correct. No conclusions should be drawn from any statistical test without checking the appropriate assumptions.
...
```{r fig27, echo=FALSE, out.width="80%", fig.align="center", fig.cap="Flight times for paper helicopters when dropped from a height of 8 feet, each with a small paperclip attached at the bottom of the base of the helicopter."}
knitr::include_graphics("docs/Fig2_7.png")
```

```{r fig28, echo=FALSE, out.width="80%", fig.align="center", fig.cap="Residuals from an ANOVA and linear regression analysis of the flight times for paper helicopters from six groups: wing lengths 5.5 cm, 6.5 cm (standard), 7.5 cm, 8.5 cm, 19.5 cm, and 10.5 cm."}
