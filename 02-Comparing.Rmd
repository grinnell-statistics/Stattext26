...
>**MATHEMATICAL NOTE** In the ANOVA model, there is the appearance that we are describing two means ($\scriptstyle \mu_1$ and $\scriptstyle \mu_2$) using three parameters ($\scriptstyle \mu$, $\scriptstyle \alpha_1$ , and $\scriptstyle \alpha_2$). Since it can be shown that $\scriptstyle \alpha_2 = -\alpha_1$, there are actually just two parameters ($\scriptstyle \mu$ and $\scriptstyle alpha_1$) that are estimated. Thus, the null hypothesis stating no effect size can also be written as $\scriptstyle H_0:\  \alpha_1 \ =\  \alpha_2 = 0$ or $\scriptstyle H_0:\  \mu_1 \ =\  \mu_2 \ =\  \mu$.
...
- $\scriptstyle \bar{y}_{..}$: **grand mean** (the overall average of the combined results)
...
32. The Weight data set contains the brain weights ($\scriptstyle y$) and body weights ($\scriptstyle x$) of 32 species.  
...
**Group Mean squares ($\scriptstyle MS_{\text{Group}}$)** equals $SS_{\text{Group}}/df_{\text{Group}}$. $\scriptstyle MS_{\text{Group}}$ is a measure of variability between the levels of each factor and is often called **between-level variability**. It is actually just the variance of the level means.
