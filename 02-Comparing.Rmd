...
>**Key Concept** In testing the difference in two population means, testing the null hypothesis $\scriptstyle H_0 : \beta_1 = 0$ for a regression model is equivalent to testing the two-sample t-test hypothesis $\scriptstyle H_0 : \mu_1 - \mu_2 = 0$ *when using the equal variance assumption*.
...
>**MATHMATICAL NOTE** In the ANOVA model, there is the appearance that we are describing two means ($\scriptstyle \mu_1$ and $\scriptstyle \mu_2$) using three parameters ($\scriptstyle \mu$, $\scriptstyle \alpha_1$ , and $\scriptstyle \alpha_2$). Since it can be shown that $\scriptstyle \alpha_2 = -\alpha_1$, there are actually just two parameters ($\scriptstyle \mu$ and $\scriptstyle alpha_1$) that are estimated. Thus, the null hypothesis stating no effect size can also be written as $\scriptstyle H_0: \alpha_1 = \alpha_2 = 0$ or $\scriptstyle H_0: \mu_1 = \mu_2 = \mu$.
...
estimate $\scriptstyle \pm$ critical value $\scriptstyle \times$ standard error of the estimate
