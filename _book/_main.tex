...
\setcounter{enumi}{3}
\item
  Solve Equation (6.5) for \(\scriptstyle \pi_i\) to show that Equation (6.6) is true.
\item
  Use Equation (6.6) to create six graphs. In each graph, plot the explanatory variable (\(\scriptstyle x\)) versus the expected probability of success (\(\scriptstyle \pi\)) using \(\scriptstyle \beta_0\) = \$-\$10 and \$-\$5 and \(\scriptstyle \beta_1\) = 0.5, 1, and 1.5. Repeat the process for \(\scriptstyle \beta_0\) = 10 and 5 and \(\scriptstyle \beta_1\) = -0.5, \$-\$1, and \$-\$1.5.

  \begin{enumerate}
  \def\labelenumii{\alph{enumii}.}
  \tightlist
  \item
    Do not submit the graphs, but explain the impact of changing \(\scriptstyle \beta_0\) and \(\scriptstyle \beta_1\).\\
  \item
    For all of these graphs, what value of \(\scriptstyle \pi\) appears to have the steepest slope?
  \end{enumerate}
\item
  Use statistical software to calculate the maximum likelihood estimates of \(\scriptstyle \beta_0\) and \(\scriptstyle \beta_1\). Compare the maximum likelihood estimates to the least squares estimates in Question 3.
\end{enumerate}
...
\tightlist
\item
  Using Radius as the first explanatory variable, \(\scriptstyle x_1\), and Concave as the second explanatory variable, \(\scriptstyle x_2\), submit the logistic regression model. In other words, find the coefficients for the model
\end{enumerate}
...
\textbf{\textcolor{blue}{Key Concept:}}
\textcolor{blue}{To use the drop-in-deviance test to determine if $\scriptstyle x_i$ should be included in a model:}
...
\begin{quote}
\textcolor{blue}{2. Calculate the deviance (or $\scriptstyle -2 \  \times$ log-likelihood) for the reduced (i.e., restricted) model (e.g., the model including all the variables in step 1 except for $\scriptstyle x_i$).}
\end{quote}

\begin{quote}
\textcolor{blue}{3. Calculate $\scriptstyle G \ =\  2 \times$ log-likelihood (full model) $\scriptstyle -2 \ \times$ log-likelihood (restricted model) = deviance (restricted model) $-$ deviance (full model).}
\end{quote}
...
\tightlist
\item
  What are the maximum likelihood estimates \(\scriptstyle b_0\) and \(\scriptstyle b_1\)?\\
\item
  Substitute \(\scriptstyle b_0\) and \(\scriptstyle b_1\) into Equation (6.9) to estimate the probability of malignancy when the radius is 4.5.\\
\item
  Use Wald's test and the \(\scriptstyle G\)-statistic to determine if this model provides evidence that the probability of malignancy is related to cell radius.
\end{enumerate}
...
(c) Find the \(\scriptstyle p\)-value by using software or looking up the statistic in a chi‐square probability distribution table.\\
(d) Interpret the results and clearly state how your conclusions are impacted by random sampling and random allocation to treatments in the original study design. Remember that a small \(\scriptstyle p\)-value provides evidence that the model is \emph{not} a good fit.
...
\tightlist
\item
  Calculate the deviance chi‐square goodness‐of‐fit test by hand for the Cancercells data given in Table 6.3. Expected counts, degrees of freedom, and \(\scriptstyle p\)-values are found the same way as in the Pearson test. The only difference is in the calculation of the test statistic:
\end{enumerate}
...
\tightlist
\item
  For binomial response data that follow the binomial distribution, the log-likelihood function for one observation (\(\scriptstyle y_1\)) is
\end{enumerate}
...
Where \(\scriptstyle C\) is a constant that does not influence \(\scriptstyle \pi_1\), we can drop \(\scriptstyle C\) from the above equation without any impact on the maximum likelihood estimate. Assume that you observed \(\scriptstyle y_1\) = 5 malignant cells out of a sample of \(\scriptstyle n_1\) = 12. Substituting these values into Equation (6.32) and dropping \(\scriptstyle C\) simplifies the log-likelihood function to \(\scriptstyle \ln[P(Y_1 \ = \ 5)] \ = \ 5\ln(\pi_1) \ + \  7\ln(1 \ - \ \pi_1).\)
a. Plot the log-likelihood function for several values of \(\scriptstyle \pi_1\) between 0 and 1. Use the plot to estimate the value of \(\scriptstyle \pi_1\) that will maximize the log-likelihood function.\\
b. If you have had calculus, set the derivative of the log-likelihood function to 0 and solve for \(\scriptstyle \pi_1\). What is the maximum likelihood estimate for \(\scriptstyle \pi_1\)?
...
\setcounter{enumi}{5}
\item
  Consider the chocolate chip melting time data in Table 9.2. What is \(\scriptstyle m\)? List \(\scriptstyle t_1\) through \(\scriptstyle t_m\) for the chip melting times.
\item
...
Table 9.3 displays some of the quantities required to compute the Kaplan--Meier estimates. After the time intervals, the number of events of interest (\(d_i\)) and the number at risk (\(\scriptstyle n_i\)) have been calculated, three more calculations are still needed for each interval: \(\scriptstyle \hat p_i\), 1 - \(\scriptstyle \hat p_i\), and \(\scriptstyle \hat S(t_i)_{\text{KM}}\).
...
Now if \(\scriptstyle \hat p_i\) is the probability of an individual experiencing the event in the \texttt{i}th time interval, given that the individual has not experienced the event in the previous time intervals, then 1 - \(\scriptstyle \hat p_i\) {[}{[}{[}{[}{[}{]}{]}{]}{]}{]} i is the probability of \emph{not} experiencing the event (i.e., \emph{surviving}) through the \texttt{i}th time interval, given that the individual has not experienced the event prior to the \texttt{i}th time interval.
...
\setcounter{enumi}{8}
\item
  What is the value of \(\scriptstyle \hat p_1\)? Interpret this value.
\item
...
\item
  Calculate the remaining estimated conditional probabilities \(\scriptstyle \hat p_3\) and \(\scriptstyle \hat p_4\). Place these values in the appropriate cells in Table 9.3 and interpret the values.\\
\item
  Calculate the remaining estimated conditional probabilities 1 - \(\scriptstyle \hat p_3\) and 1 - \(\scriptstyle \hat p_4\). Place these values in the appropriate cells in Table 9.3 and interpret the values.
\end{enumerate}
...
\item
  What is the estimate for \(\scriptstyle S(45)\) in Table 9.3? That is, what proportion of chips in the sample has not melted after 45 seconds?
\item
...
\item
  Assume that no censoring is present in the melting times (see the entries in Table 9.1). Estimate \texttt{S(25)}, \texttt{S(30)}, \texttt{S(45)}, and \texttt{S(55)} using both the empirical survival function and the Kaplan--Meier estimator, and compare your answers. What do your answers suggest about the Kaplan--Meier estimator when no censoring is present?
\end{enumerate}
...
Once the survival probabilities have been estimated, a graph of the Kaplan--Meier curve can be constructed to display the relationship between time and the estimated probability of surviving. The Kaplan--Meier curve is an approximation to the true survival curve, which is a graphical representation of \texttt{S(t)}. The values of \(\scriptstyle \hat S(t)_{\mathrm{KM}}\) are plotted against the complete event times \(\scriptstyle t_1, \ t_2, \ \dots, \ t_m\). Figure 9.1 shows the Kaplan--Meier curve for the chocolate chip melting times displayed in Table 9.2.

Notice that the value of \(\scriptstyle \hat S(30)_{\mathrm{KM}}\) remains the same across each time interval. From Table 9.3, we see that, for example, \(\scriptstyle \hat S(30)_{\mathrm{KM}} = 5/7\); i.e., the proportion of chips in the sample that have not melted after 30 seconds is 5/7, or 71\%. This value remains constant for time \texttt{t} in the interval {[}30,45). From Figure 9.1 we can observe that the Kaplan--Meier curve is a decreasing series of steps, with drops occurring at each complete event time \(\scriptstyle t_i\). This type of plot is called a \emph{step function} because it looks like a series of steps. The height of each step corresponds to the value of \(\scriptstyle \hat S(t_i)_{\mathrm{KM}}\) for \texttt{t} inside {[}\(\scriptstyle t_i\), \(\scriptstyle t_{i+1}\)), where \(\scriptstyle i = 0,\ \dots,\ m-1\) (with the convention that \(t_0 = 0\)).
...
\setcounter{enumi}{17}
\item
  Compare the values of \(\scriptstyle \hat S(t)_{\mathrm{KM}}\) in Table 9.3 to those in Figure 9.1. How would the Kaplan--Meier curve in Figure 9.1 change if the largest observed melting time were not censored?
\item
...
Equations \ref{9.3} and \ref{9.4} can be thought of as a weighted average of the time-to-event data, where the Kaplan--Meier curve \(\scriptstyle \hat S(t_i)_{\mathrm{KM}}\) acts as weights (probabilities) for the time-to-event intervals. Each term in the summation of Equations \ref{9.3} and \ref{9.4} represents the area of a rectangle formed by the width of the \texttt{i}th time interval \(\scriptstyle t_{i+1} \ -\  t_i\) and the height of the Kaplan--Meier curve \(\scriptstyle \hat S(t_i)_{\mathrm{KM}}\). For example, the width of the left-most rectangle in Figure 9.3 outlined by the vertical dotted line is 25 seconds, and the height of the rectangle is 1. So the first term in the sum of Equation \ref{9.3} is \(\scriptstyle \hat S(t_0)_{\mathrm{KM}}(25 - 0) = (1)(25) = 25\).
...
For example, to find the estimate for the 70th percentile of the chocolate chip melting times in Figure 9.1, we find the smallest complete event time \(\scriptstyle t_i\) such that at least 70\% of the chips have melted: Since \(\scriptstyle \hat S(t_i)_{\mathrm{KM}} \ \le\) 1 - 0.7 = 0.3, we draw a horizontal line at \(\scriptstyle \hat S(t)\) = 0.3, and eventually we reach the vertical step that occurs at \(\scriptstyle t_4\) = 55. Since \(\scriptstyle t_4\) = 55 is the smallest complete event time that satisfies \(\scriptstyle \hat S(t_i)_{\mathrm{KM}} \ \le\) 0.3, \(\scriptstyle\hat t_{(70)}\) = 55. See Figure 9.4.
...
\setcounter{enumi}{22}
\item
  For the chocolate chip melting time data in Table 9.2, calculate the estimated median survival time\(\scriptstyle hat t_{(50)}\). Since right-censored data are typically right skewed, the median survival time is usually preferred to the mean.
\item
...
\begin{quote}
\textbf{MATHEMATICAL NOTE} Based on the definition of the estimated percentile in Equation \ref{9.5}, it is possible that particular estimated percentiles do not exist. The \(p\)th percentile will not exist (and your software may return NA) if there does not exist a complete time \(\scriptstyle t_p\) such that \(\scriptstyle \hat S(t_p)_{\mathrm{KM}} \ \le\  1 \ -\  \tfrac{p}{100}\). For example, in the MeltingChipsJS data set, more than 25\% of the milk chocolate chips had not melted by the end of the study (75 seconds). Thus, there is no estimate of the 75th percentile and the IQR cannot be calculated.
\end{quote}
...
With the computed standard error, we can use Formula \ref{9.7} to construct a confidence interval for a survival probability. For example, the 95\% confidence interval for \texttt{S(25)}---the probability that a randomly selected milk chocolate chip takes longer than 25 seconds to melt---is calculated as
...
\item
  Using your above answers and the appropriate entries in Table 9.3, construct 95\% confidence intervals for the survival probabilities \texttt{S(t)} at each of the remaining complete melting times, and interpret their values.
\item
...
\begin{quote}
\textbf{NOTE} The confidence interval for \texttt{S(t)} is valid for a single fixed time at which the inference is to be made. For this reason, confidence intervals are sometimes referred to as \emph{pointwise intervals}. Each interval constructed using Formula \ref{9.7} is relevant only for \texttt{t} within the \texttt{i}th interval. Therefore, it is not correct to state that we are 95\% confident that the entire true survival curve \texttt{S(t)} falls between the confidence bands in Figure 9.5. If we want a confidence band for an entire survival function \texttt{S(t)} within which we can guarantee with a specified level of confidence that the entire curve falls, alternative confidence bands need to be computed.
\end{quote}
...
\tightlist
\item
  Calculate the missing quantities in Table 9.5. Note that \(\scriptstyle E_{19}\), \(\scriptstyle E_{1,10}\), \(\scriptstyle V_{19}\), and \(\scriptstyle V_{1,10}\) are all equal to 0. This is because all the milk chocolate chips have melted by 62 seconds.\\
\item
...
Equation \ref{9.13} may look something like the formal definition of the derivative of some function (although it probably isn't clear what that function is). It turns out that \texttt{h(t)} is related to the derivative of a function of the (population) survival function \texttt{S(t)} by the expression
...
The estimates of hazard are based on quantities used to find the Kaplan--Meier estimated probabilities, so we will first construct time intervals \(\scriptstyle [t_i,\,t_{i+1})\) for \(\scriptstyle i = 1,\dots,m-1\) as if we were constructing the Kaplan--Meier estimator, where \(\scriptstyle t_0\) = 0 and \(\scriptstyle m\) is the number of complete event times (see Section 9.3 for details).
...
From the Kaplan--Meier curve in Figure 9.10, we can observe that the estimated proportion of individuals who had not taken their first drink decreases rapidly after age 13. What does this mean in terms of the estimated hazard rates? Examine Figure 9.10, which was produced using the R statistical software. Part (a) displays the Kaplan--Meier curve \(\scriptstyle \hat S(t)_{\mathrm{KM}}\), while Part (b) displays the estimated hazard curve \(\scriptstyle \hat h(t)_{\mathrm{KM}}\).
...
\section{\texorpdfstring{\textcolor{blue}{The Cumulative Hazard}}{}}\label{section-85}}
...
interval {[}\(\scriptstyle t_m\), \(\scriptstyle t_n\)) (see Figure 9.14 for example).
...
\tightlist
\item
  Use Equation \ref{9.17} and the quantities in Table 9.6 to compute \(\scriptstyle \widehat H(t)_{\mathrm{NA}}\) for the chip melting times from Table 9.2 by hand.
\end{enumerate}
...
Figure 9.14 displays the estimated hazard curve for the chip melting times (shown earlier in Figure 9.9) in Part (a) and the estimated cumulative hazard function in Part (b). Because of the small number of intervals, it is difficult to determine if the rate of change in \(\scriptstyle \widehat H(t)_{\mathrm{NA}}\) is increasing or decreasing.
