...
\textbf{\textcolor{blue}{Key Concept:}} \textcolor{blue}{The calculation of an empirical $\scriptstyle p$-value requires these steps:}
...
\begin{enumerate}
  ...
  \item Compare the shape, center, and spread of the simulated histograms from Part B and Question 21 Part A. Are they similar?
...
\begin{enumerate}
 ...
 \item Create a bootstrap distribution of the mean by taking 1000 resamples (with replacement). Create a bootstrap t confidence interval and a bootstrap percentile distribution to estimate the mean salaries.
  ...
  \item Use Formula (1.2) to create a 95$\%$ confidence interval for the mean. Compare this confidence interval to those in Part A. Would you expect these intervals to be similar? Why or why not?
...
\begin{enumerate}
 ...
 \item Find 2 X P(W $\scriptstyle \leq$ 18) assuming W ~ N(27.5, 4.787). How does your answer compare to that from Question 25?
...
\item
  ...
  actual experiment.
\item
  X/N is computed to find the \(\scriptstyle p\)-value, the proportion of times the statistic exceeds or is the same as the
  observed difference.
\end{itemize}
...
\item
  A \textbf{bootstrap t confidence interval} is found by calculating \(\scriptstyle \bar{x}\) \(\scriptstyle \pm t^*(S^*)\) where \(\scriptstyle S^*\) is the standard
  deviation of the bootstrap distribution and \(\scriptstyle t^*\) is the critical value of the t(n - 1) distribution with
  100(1 - \(\scriptstyle \alpha\))\%, of the area between - t* and t*.
\item
...
of the simulation results, the $\scriptstyle p$-value, and a summary statement indicating your conclusion about the research question of schistosome viability.
...
Take a simple random sample of size n = 40 from the ChiSq data. Create a bootstrap distribution of the 25% trimmed mean by taking 1000 resamples (with replacement). In other words, for each resample calculate the mean of the middle 20 observations (remove the smallest 10 and largest 10 values in each resample). Create a histogram of the 1000 trimmed means and describe the shape of this bootstrap distribution. Create a bootstrap t confidence interval and a bootstrap percentile confidence interval to estimate the 25% trimmed mean.
  ...
  \begin{enumerate}
    ...
    \item What is the individual critical value if you use the Bonferroni method with an overall (familywise) $\scriptstyle \alpha$-level of 0.05? Do any of your previous conclusions in the preceding parts of this exercise change if you test for an overall (familywise) comparison? Explain.
  \end{enumerate}
...
The key question in this study is whether or not the two types of games have different average completion times. The two-sample t-test starts with the assumption that the two group means are equal. This is often written as the null hypothesis \(\scriptstyle H_0 : \mu_1 - \mu_2 = 0\) or, equivalently, \(\scriptstyle H_0 : \mu_1 = \mu_2\).

The underlying model used in the two-sample t-test is designed to account for these two group means (\(\scriptstyle \mu_1\) and \(\scriptstyle \mu_2\)) and random error. The statistical model for the first population, the color distracter group, is:
...
\tightlist
\item
  Constant parameters: The population values in this model (\(\scriptstyle \mu_1\) , \(\scriptstyle \mu_2\) , and \(\scriptstyle \sigma\)) do not change throughout the study.
\item
...
\item
  \(\scriptstyle \epsilon_{i, j} \sim N(0,\sigma^2)\). This assumption has many key components:
\item
...
\item
  The population variance \(\scriptstyle \sigma^2\) is the same for both groups (color distracter and standard games) being tested.
\end{itemize}
...
\item
  The error terms in the regression model are assumed to come from a single population with variance \(\scriptstyle \sigma^2\) (i.e., the variance does not depend on \(\scriptstyle x\)).
\end{itemize}
...
\tightlist
\item
  \(\scriptstyle y_{i,j}\): observed completion time for subject j from group i
\item
  \(\scriptstyle \mu\): overall mean (the benchmark value)
\item
  \(\scriptstyle \alpha_i\): effect of group i (i = 1, 2)
\item
  \(\scriptstyle \epsilon_{i,j}\): error for the jth subject ( \(\scriptstyle j = 1, 2, ... , 20\)) from the ith group (\(\scriptstyle i = 1, 2\))
\end{itemize}
...
\tightlist
\item
  \(\scriptstyle \mu_1 = \mu + \alpha_1\): population mean for the color distracter games
\item
  \(\scriptstyle \mu_2 = \mu + \alpha_2\): population mean for the standard games
\end{itemize}
...
\begin{quote}
\textbf{MATHEMATICAL NOTE} In the ANOVA model, there is the appearance that we are describing two means (\(\scriptstyle \mu_1\) and \(\scriptstyle \mu_2\)) using three parameters (\(\scriptstyle \mu\), \(\scriptstyle \alpha_1\) , and \(\scriptstyle \alpha_2\)). Since it can be shown that \(\scriptstyle \alpha_2 = -\alpha_1\), there are actually just two parameters (\(\scriptstyle \mu\) and \(\scriptstyle alpha_1\)) that are estimated. Thus, the null hypothesis stating no effect size can also be written as \(\scriptstyle H_0:\  \alpha_1 \ =\  \alpha_2 = 0\) or \(\scriptstyle H_0:\  \mu_1 \ =\  \mu_2 \ =\  \mu\).
\end{quote}
...
\tightlist
\item
  \(\scriptstyle \bar{y}_{..}\): \textbf{grand mean} (the overall average of the combined results)
\item
  \(\scriptstyle \bar{y}_{1.}\): average for the color distracter game sample results
\item
  \(\scriptstyle \bar{y}_{2.}\): average for the standard game sample results
\end{itemize}
...
The model assumptions for ANOVA are equivalent to those for the two previous tests. In fact, the assumptions discussed in this section are called the six \emph{Fisher assumptions}, after Ronald Fisher, who developed the ANOVA and the corresponding \(\scriptstyle F\)-test.

\begin{itemize}
\tightlist
\item
  The parameters (\(\scriptstyle \mu\), each \(\scriptstyle \alpha_i\), and \(\scriptstyle \sigma^2\)) are constant throughout the study.
\item
...
\tightlist
\item
  The Weight data set contains the brain weights (\(\scriptstyle y\)) and body weights (\(\scriptstyle x\)) of 32 species.
\end{enumerate}
...
\textbf{Group Mean squares (\(\scriptstyle MS_{\text{Group}}\))} equals \(SS_{\text{Group}}/df_{\text{Group}}\). \(\scriptstyle MS_{\text{Group}}\) is a measure of variability between the levels of each factor and is often called \textbf{between-level variability}. It is actually just the variance of the level means.
...
\begin{quote}
\textbf{MATHEMATICAL NOTE} For any regression equation of the form \(\scriptstyle y_i = \beta_0 + \beta_1x_i + \epsilon_i\) the hypothesis test for the slope of the regression equation (b1) is similar to other t-tests discussed in introductory textbooks. (Mathematical details for this hypothesis test are described in Chapter 2.)
\end{quote}
...
\def\labelenumi{\alph{enumi}.}
\item
  Calculate seven regression models, each with one of the following explanatory variables: Cyl, Liter, Doors, Cruise, Sound, Leather, and Mileage. Identify the explanatory variable that corresponds to the model with the largest \(\scriptstyle {R}^2\) value. Call this variable \(\scriptstyle X_1\).
\item
  Calculate six regression models. Each model should have two explanatory variables, \(\scriptstyle X_1\) and one of the other six explanatory variables. Find the two-variable model that has the highest \(\scriptstyle {R}^2\) value. How much did \(\scriptstyle {R}^2\) improve when this second variable was included?
\item
...
\tightlist
\item
  The model parameters \(\scriptstyle (\beta_0 + \beta_1x_{1,i} + {...} + \beta_{p-1})\) and \(\scriptstyle \sigma\) are constant.
\item
...
\item
  The error terms follow a normal probability distribution centered at zero with a fixed variance, \(\scriptstyle \sigma^2\). This assumption is denoted as \(\scriptstyle \epsilon_i \sim N(0,\sigma^2)\) for \(\scriptstyle i = 1, {...}, ~ n\).
\end{itemize}
...
\tightlist
\item
  Transform the suggested retail price to log (Price) and \(\scriptstyle \sqrt{Price}\). Transforming data using roots, logarithms, or reciprocals can often reduce heteroskedasticity and right skewness. (Transformations are discussed in Chapter 2. Create regression models and residual plots for these transformed response variables using the explanatory variables selected in Question 5.
\end{enumerate}

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Which transformation did the best job of reducing the heteroskedasticity and skewness in the residual plots? Give the \(\scriptstyle {R}^2\) values of both new models.
\item
  Do the best residual plots correspond to the best \(\scriptstyle {R}^2\) values? Explain.
...
\tightlist
\item
  Create three regression models using Price as the response variable. In all three cases, provide the regression model, \(\scriptstyle {R}^2\) value, t-statistic for the slope coefficients, and corresponding \(\scriptstyle p\)-values.
\end{enumerate}
...
\item
  In the third model, use Mileage, Liter, and number of cylinders (Cyl) as the explanatory variables. How did the test statistics and \(\scriptstyle p\)-values change when all three explanatory variables were included in the model?
\end{enumerate}
...
\section{\texorpdfstring{\textcolor{blue}{F-Tests for Multiple Regression}}{}}\label{section-33}}
...
The key point of the previous calculations is to show that the total variability in the response, \(\scriptstyle \sum_{i=1}^n (y_i - \bar{y})^2\), can be decomposed into the following:
...
\section{\texorpdfstring{\textcolor{blue}{Developing a Model to Confirm a Theory}}{}}\label{section-34}}
...
\section{\texorpdfstring{\textcolor{blue}{Interaction and Terms for Curvature}}{}}\label{section-35}}
...
\def\labelenumi{(\alph{enumi})}
\item
  Does the \(\scriptstyle R^2_{\text{adj}}\) value increase when the interaction term is added? Based on the change in \(\scriptstyle R^2_{\text{adj}}\), should the interaction term be included in the model?
\item
...
\tightlist
\item
  Use the \texttt{4-8Cyl} data set to calculate the regression line \(\scriptstyle Price = \beta_0 + \beta_1(Mileage) + \beta_3(Cadillac) + \beta_4(SAAB)\). You will need to create indicator variables for \texttt{Make} before calculating the regression line.
\end{enumerate}

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Create a scatterplot with \texttt{Mileage} as the explanatory variable and \texttt{Price} as the response. Overlay a second graph with \texttt{Mileage} as the explanatory variable and \(\scriptstyle \hat y\) as the response. Notice that the predicted values (the \(\scriptstyle \hat y\) values) form two separate lines. Do the parallel lines (no interaction model) look appropriate?
\item
...
\section{\texorpdfstring{\textcolor{blue}{A Closer Look at Variable Selection Criteria}}{}}\label{section-36}}
...
\begin{quote}
\textbf{MATHEMATICAL NOTE} Intuitively, each additional term in a regression model means that one additional parameter value must be estimated. Each parameter estimate costs an additional degree of freedom. Thus, \(\scriptstyle R^2_{adj}\) is an \(\scriptstyle {R}^2\) value that is adjusted for degrees of freedom and can be written as
\end{quote}
...
\(\scriptstyle R^2_{adj}\) measures the spread of the residuals using MSE, while \(\scriptstyle {R}^2\) measures the spread of the residuals using SSE.
...
The \(\scriptstyle C_p\) statistic assumes that \(\scriptstyle \hat{\sigma}^2_{Full}\) is an unbiased estimate of the overall residual variability, \(\scriptstyle \sigma^2\). If the full model has several terms that are not useful in predicting the response (i.e., several coefficients are essentially zero), then \(\scriptstyle \hat{\sigma}^2_{Full}\) will overestimate \(\scriptstyle \sigma^2\) and \(\scriptstyle C_p\) will be small.*
...
\section{\texorpdfstring{\textcolor{blue}{Chapter Summary}}{}}\label{section-37}}
...
\tightlist
\item
  \(\scriptstyle n\) is the sample size
\item
  \(\scriptstyle p\) is the number of coefficients in the model (including \(\scriptstyle \beta_0\))
\item
  \(\scriptstyle \hat{\\sigma}^2 = \frac{\\sum_{i=1}^n (y_i - \hat{y}_i)^2}{n - p}\) estimates the variance of the residuals in the current model tested.
\item
  \(\scriptstyle \hat{\sigma}_{\text{Full}}^2\) estimates the variance of the residuals in the full model (the model with all explanatory variables).
\end{itemize}
...
\section{\texorpdfstring{\textcolor{blue}{Investigation: Which Microwave Popcorn Is the Best?}}{}}\label{section-38}}
...
\section{\texorpdfstring{\textcolor{blue}{Elements of a Well-Designed Experiment}}{}}\label{section-39}}
...
\section{\texorpdfstring{\textcolor{blue}{Analyzing a Two-Way Factorial Design}}{}}\label{section-40}}
...
\subsection[Comparing Variances \{-\} ]{\texorpdfstring{Comparing Variances \{-\} \footnote{If you have studied ANOVA tables before, you may find it surprising that we focus on mean squares (MS) and do not discuss sums of squares (SS) or degrees of freedom (df). The focus of this section is the concepts and logic behind ANOVA. ANOVA is the process of comparing between group and within group variability. These types of variability are represented by the mean squares. Chapter 2 and later Chapter 4 activities discuss sums of squares and degrees of freedom.}}{Comparing Variances \{-\} }}\label{comparing-variances--}}
...
\item
  Use the technology instructions provided on the CD to check your answers. Submit the software output. Note that a \(\scriptstyle p\)-value for each \(\scriptstyle F\)-statistic is provided. State your conclusions about each of the three hypotheses based on these \(\scriptstyle p\)-values.\\
...
\section{\texorpdfstring{\textcolor{blue}{Analyzing a Three-Way Factorial Design}}{}}\label{section-41}}
...
\item
  Use the \(\scriptstyle p\)-value corresponding to each hypothesis to state your conclusions.
\item
...
\tightlist
\item
  Determine whether \(\scriptstyle MS_{\text{Brand}}\), \(\scriptstyle MS_{\text{Time}}\), and \(\scriptstyle MS_{\text{BrandTime}}\) are the same as in Question 13. Explain why the \(\scriptstyle F\)-statistics corresponding to these three hypotheses have changed.
\end{enumerate}
...
\section{\texorpdfstring{\textcolor{blue}{What Can We Conclude from the Popcorn Study?}}{}}\label{section-42}}
...
\section{\texorpdfstring{\textcolor{blue}{Paper Towels: Developing a Statistical Model for a Two-Way Factorial Design}}{}}\label{section-43}}
...
\addcontentsline{toc}{subsection}{\textcolor{blue}{Practice:} Algebraic Notation}
...
\tightlist
\item
  \(\scriptstyle y_{ijk}\): the \(\scriptstyle k\)th observed breaking strength (\(\scriptstyle k = 1, 2, \ldots, 26\)) for brand \(\scriptstyle i\) and water amount \(\scriptstyle j\)
\item
  \(\scriptstyle \mu\): overall mean breaking strength of the entire population of paper towels across brands and water amounts (also called the grand mean)
\item
  \(\scriptstyle \alpha_i\): brand effect (i = 1, 2), where \(\scriptstyle \alpha_1\) is the effect of Brand C and \(\scriptstyle \alpha_2\) is the effect of Brand D
\item
  \(\scriptstyle \beta_j\): amount of water effect (j = 1, 2, 3), where \(\scriptstyle \beta_1\) represents the effect of 0 drops of water
\item
  \(\scriptstyle (\alpha\beta)_{ij}\): interaction effect, where \(\scriptstyle (\alpha\beta)_{23}\) represents the Brand D/15 drops of water interaction effect
\item
  \(\scriptstyle \varepsilon_{ijk}\): the random error---the difference between the \(\scriptstyle k\)th observed value (k = 1, 2, \(\ldots\), 26) and the population mean breaking strength for brand \(\scriptstyle i\) and water amount \(\scriptstyle j\)
\end{itemize}
...
\section{\texorpdfstring{\textcolor{blue}{Paper Towels: The Relationship Between Effects and ANOVA}}{}}\label{section-44}}
...
\tightlist
\item
  Table 4.6 is a table with two rows and three columns, similar to the interaction effect term in the two-way factorial diagram in Figure 4.4. However, for this question we will assume that only two effects are known: \(\scriptstyle \widehat{(\alpha\beta)}_{11} = 2\) and \(\scriptstyle \widehat{(\alpha\beta)}_{12}\) = -5.
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  Equation \ref{4.8} states that all the \(\scriptstyle AB\) effects within Brand C add up to zero {[}\(\scriptstyle \widehat{(\alpha\beta)}_{11} \ +\  \widehat{(\alpha\beta)}_{12} \ +\  (\widehat{(\alpha\beta)}_{13}\) = 0{]}. Use this rule to calculate \(\scriptstyle \widehat{(\alpha\beta)}_{13}\).
\item
  Equation \ref{4.8} also states that all the \(\scriptstyle AB\) effects within 0 water amount add up to zero (the same for 5 and 15 drops of water). Use this rule to calculate \(\scriptstyle \widehat{(\alpha\beta)}_{21}\), \(\scriptstyle \widehat{(\alpha\beta)}_{22}\), and \(\scriptstyle \widehat{(\alpha\beta)}_{23}\).
\item
  Consider a different interaction table with two rows and three columns. Explain why it is not possible to have effects of \(\scriptstyle \widehat{(\alpha\beta)}_{11} = 4\), \(\scriptstyle \widehat{(\alpha\beta)}_{13} = -4\), and \(\scriptstyle \widehat{(\alpha\beta)}_{22}\) = 6 and still follow the restrictions in Equation 4.8.
\item
...
\tightlist
\item
  Table 4.7 is another table of interaction effects, with five rows and three columns (five levels of factor \(\scriptstyle A\) and three levels of factor \(\scriptstyle B\)). Again, we will assume that only some of the effects are known.
\end{enumerate}
...
\tightlist
\item
  Use the previous two questions to determine the degrees of freedom for an interaction term for a balanced completely randomized design using three levels of factor \(\scriptstyle A\) and four levels of factor \(\scriptstyle B\).
\end{enumerate}
...
\tightlist
\item
  \textbf{Checking Assumptions} In the statistical model in Equation \ref{4.4}, the following assumptions need to be validated about the random error terms, \(\scriptstyle \varepsilon_{ijk}\), before any formal hypothesis test can be developed:
\end{enumerate}
...
\item
  The error terms follow a normal probability distribution, denoted as \(\scriptstyle \epsilon \sim N(0, \sigma^2)\).
\end{itemize}

Note that the second assumption includes an equal variance assumption about the random errors from the different factor-level groups: \(\scriptstyle \sigma_{11}^2 \ =\  \sigma_{12}^2 \ =\ \sigma_{13}^2 \ =\  \sigma_{21}^2 \ =\  \sigma_{22}^2 \ =\  \sigma_{23}^2\).
...
\item
  Transform the response variable using natural log (\(\scriptstyle \log(\text{Strength})\)) and \(\scriptstyle \sqrt{\text{Strength}}\). Did both transformations improve the equal variance assumption?
\end{enumerate}
...
\tightlist
\item
  \textbf{Analyzing the Data} Use computer software to conduct an analysis on the square-root transformed \texttt{PaperTowels} data to test for differences between brands, water levels, and interactions. Use the ANOVA as well as appropriate graphs to state your conclusions about the paper towel study.
\end{enumerate}

\hypertarget{section-45}{%
\section{\texorpdfstring{\textcolor{blue}{Contrasts and Multiple Comparisons}}{}}\label{section-45}}
...
\textbf{\textcolor{blue}{Key Concept:}} \textcolor{blue}{For any null hypothesis test comparing $G$ group means $H_0$: $\mu_1 = \mu_2 = \cdots = \mu_G$ versus the alternative $H_a$: at least one group mean is different from another, a contrast is an estimate of a linear combination of the group means: Contrast 1 ($C_1$) = ($C_{11}$)$\bar{y}_1$ + ($C_{21}$)$\bar{y}_2$ + $\cdots$ + ($C_{G1}$)$\bar{y}_G$, where the coefficients sum to zero: $C_{11} + C_{21} + \cdots + C_{G1} = 0$.}
...
\item
  Compare \(\scriptstyle MS_{C2}\) and \(MS_{C3}\), the mean squares found in Question 38.
\end{enumerate}
...
\section{\texorpdfstring{\textcolor{blue}{Chapter Summary}}{}}\label{section-46}}
...
\section{\texorpdfstring{\textcolor{blue}{Investigation: Is Cell Shape Associated with Malignancy?}}{}}\label{section-47}}
...
\section{\texorpdfstring{\textcolor{blue}{Summarizing Categorical Data}}{}}\label{section-48}}
...
\section{\texorpdfstring{\textcolor{blue}{A Simulation Study: How Likely Is It That the Observed Sample Would Occur by Chance?}}{}}\label{section-49}}
...
\section{\texorpdfstring{\textcolor{blue}{Fisher’s Exact Test}}{}}\label{section-50}}
...
\section{\texorpdfstring{\textcolor{blue}{Two-Sided Hypothesis Tests}}{}}\label{section-51}}
...
\section{\texorpdfstring{\textcolor{blue}{Chi-Square Test}}{}}\label{section-52}}
...
\section{\texorpdfstring{\textcolor{blue}{What Can We Conclude from the Cancer Study?}}{}}\label{section-53}}
...
\section{\texorpdfstring{\textcolor{blue}{Relative Risk and the Odds Ratio}}{}}\label{section-54}}
...
\section{\texorpdfstring{\textcolor{blue}{Sampling Designs}}{}}\label{section-55}}
...
\section{\texorpdfstring{\textcolor{blue}{Comparing Tests of Homogeneity and Independence}}{}}\label{section-56}}
...
\section{\texorpdfstring{\textcolor{blue}{Chi-Square Goodness-of-Fit Tests}}{}}\label{section-57}}
...
\section{\texorpdfstring{\textcolor{blue}{Chapter Summary}}{}}\label{section-58}}
...
\begin{verbatim}
## WARNING: Rtools is required to build R packages, but no version of Rtools compatible with R 4.5.0 was found. (Only the following incompatible version(s) of Rtools were found: 4.2)
## 
...
\section{\texorpdfstring{\textcolor{blue}{ Investigation: Did Temperature Influence the Likelihood of an O-Ring Failure?}}{}}\label{section-59}}
...
\section{\texorpdfstring{\textcolor{blue}{ Review of the Least Squares Regression Model}}{}}\label{section-60}}
...
\section{\texorpdfstring{\textcolor{blue}{ The Logistic Regression Model}}{}}\label{section-61}}
...
\section{\texorpdfstring{\textcolor{blue}{ The Logistic Regression Model Using Maximum Likelihood Estimates}}{}}\label{section-62}}
...
\section{\texorpdfstring{\textcolor{blue}{ Interpreting the Logistic Regression Model}}{}}\label{section-63}}
...
\section{\texorpdfstring{\textcolor{blue}{ Inference for the Logistic Regression Model}}{}}\label{section-64}}
...
\section{\texorpdfstring{\textcolor{blue}{ What Can We Conclude from the Space Shuttle Study?}}{}}\label{section-65}}
...
\section{\texorpdfstring{\textcolor{blue}{ Logistic Regression with Multiple Explanatory Variables}}{}}\label{section-66}}
...
\section{\texorpdfstring{\textcolor{blue}{ The Drop-in-Deviance Test}}{}}\label{section-67}}
...
\section{\texorpdfstring{\textcolor{blue}{ Measures of Association}}{}}\label{section-68}}
...
\section{\texorpdfstring{\textcolor{blue}{ Review of Means and Variances of Binary and Binomial Data}}{}}\label{section-69}}
...
\section{\texorpdfstring{\textcolor{blue}{ Calculating Logistic Regression Models for Binomial Counts}}{}}\label{section-70}}
...
\section{\texorpdfstring{\textcolor{blue}{ Calculating Residuals for Logistic Models with Binomial }}{}}\label{section-71}}
...
\section{\texorpdfstring{\textcolor{blue}{ Assessing the Fit of a Logistic Regression Model with Binomial Counts}}{}}\label{section-72}}
...
\section{\texorpdfstring{\textcolor{blue}{ Diagnostic Plots}}{}}\label{section-73}}
...
\section{\texorpdfstring{\textcolor{blue}{ Maximum Likelihood Estimation in Logistic Regression}}{}}\label{section-74}}
...
\section{\texorpdfstring{\textcolor{blue}{ Chapter Summary}}{}}\label{section-75}}
...
\section{\texorpdfstring{\textcolor{blue}{Investigation: How Long Does It Take for Chocolate Chips to Melt?}}{}}\label{section-76}}
...
\section{\texorpdfstring{\textcolor{blue}{Overview of Survival Analysis Studies and Data}}{}}\label{section-77}}
...
\section{\texorpdfstring{\textcolor{blue}{The Survival Function}}{}}\label{section-78}}
...
\section{\texorpdfstring{\textcolor{blue}{Descriptive Statistics for Survival Data}}{}}\label{section-79}}
...
\section{\texorpdfstring{\textcolor{blue}{Confidence Intervals for Survival Probabilities}}{}}\label{section-80}}
...
\section{\texorpdfstring{\textcolor{blue}{Comparing Survival Functions}}{}}\label{section-81}}
...
\section{\texorpdfstring{\textcolor{blue}{What Can We Conclude About Melting Chocolate Chips?}}{}}\label{section-82}}
...
\section{\texorpdfstring{\textcolor{blue}{The Hazard Function}}{}}\label{section-83}}
...
\section{\texorpdfstring{\textcolor{blue}{The Cumulative Hazard}}{}}\label{section-84}}
...
\section{\texorpdfstring{\textcolor{blue}{Additional Types of Incomplete Data}}{}}\label{section-85}}
...
\section{\texorpdfstring{\textcolor{blue}{Chapter Summary}}{}}\label{section-86}}
