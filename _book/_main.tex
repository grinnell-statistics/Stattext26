...
\begin{quote}
\textbf{MATHEMATICAL NOTE} In this section, we simply discuss how to conduct Fisher's exact test with statistical software. The extended activities shows that Fisher's exact test uses the hypergeometric distribution to calculate exact \(\scriptstyle p\)-values. For any 2 × 2 contingency table when there are \(\scriptstyle N\) total observations with \(\scriptstyle M\) total successes, the probability of observing \(\scriptstyle x\) successes, \(\scriptstyle P(X \ = \ x)\), in a sample of size \(\scriptstyle n\){[}{[}{[} is{]}{]}{]}
\end{quote}
...
where \(\binom{M}{x} \ =\  M \text{“choose”} x \ =\  \frac{M!}{x!(M - x)!}\).\footnote{For any positive integer, the notation n! is read ``n factorial'' and is defined as \(\scriptstyle n!= n(n- 1)(n- 2) \cdot\cdot\cdot (3)(2)(1)\). For example, ``3 factorial'' is 3 \(\times\) 2 \(\times\) 1 = 6 and ``four factorial'' is \(\scriptstyle 4!\) = 4 \(\times\) 3 \(\times\) 2 \(\times\) 1 = 24. In addition, \(\scriptstyle 0!= 1\).} Similar calculations hold for \(\scriptstyle \binom{N - M}{n - x}\) and \(\scriptstyle \binom{N}{n}\).''
...
\begin{quote}
\textbf{MATHEMATICAL NOTE} In this section, we simply discuss how to conduct Fisher's exact test with statistical software. The extended activities shows that Fisher's exact test uses the hypergeometric distribution to calculate exact \(\scriptstyle p\)-values. For any 2 × 2 contingency table when there are \(\scriptstyle N\) total observations with \(\scriptstyle M\) total successes, the probability of observing \(\scriptstyle x\) successes, P(X = x), in a sample of size \(\scriptstyle n\) is Fisher's exact test and the corresponding simulation study were derived here using a rather strong assumption about the null hypothesis. Both tests were completed under the assumption that we had observed 13 benign and 24 malignant cells and that these totals would be the same in every randomization. Statisticians call this a \textbf{conditional test of independence}. In other words, both the row and the column totals are known (fixed) before the study is conducted. However, the extended activities will show that Fisher's exact test can be used for any 2 \(\scriptstyle \times\) 2 table, even when the margin totals are not fixed.\(^5\)
\end{quote}

\textbf{\textcolor{blue}{Key Concept:}}
\textbackslash textcolor\{blue\}\{Fisher's exact test uses the hypergeometric distribution to provide exact \(\scriptstyle p\)-values even for small sample sizes and succes/failure proportions near 0\% or 100\%\}
...
\setcounter{enumi}{14}
\item
  Repeat Question 8 to estimate the \(\scriptstyle p\)-value for the two-sided hypothesis test.
\item
  Use the hypergeometric distribution to find the \(\scriptstyle p\)-value for the two-sided hypothesis test.
\end{enumerate}

\begin{quote}
\textbf{NOTE}Two-sided tests can be somewhat cumbersome. Many statisticians suggest simply approximating a two-sided \(\scriptstyle p\)-value by doubling the one-sided \(\scriptstyle p\)-value. For example, the \(\scriptstyle p\)-value for \(\scriptstyle H_0:\  p_C \ =\  p_R\) vs.~\(\scriptstyle H_a:\  p_C \ \neq \ p_R\) is 2 \(\times\) P(X \(\ge\) 17) = 2(0.0225) = 0.045.
\end{quote}
...
\begin{quote}
\textbf{MATHEMATICAL NOTE} In this section, we simply discuss how to conduct Fisher's exact test with statistical software. The extended activities shows that Fisher's exact test uses the hypergeometric distribution to calculate exact \(p\)-values. For any 2 × 2 contingency table when there are N total observations with \(M\) total successes, the probability of observing \(\scriptstyle x\) successes, P(X = x), in a sample of size \(\scriptstyle n\) {[}{[}{[}is Some{]}{]}{]} texts suggest using the chi-square test for homogeneity if separate samples are selected from more than one population and using the chi-square test for independence when data are collected from a single sample. Even though this study is based on one sample of 37 slides, the extended activities will show that both the test of homogeneity and the test of independence are appropriate. The calculations for both tests are identical; however, the hypotheses and conclusions are different. We choose to present the test of homogeneity in this section so that the hypotheses and conclusions coincide with those for the
...
\def\labelenumi{(\alph{enumi})}
\item
  Submit the computer output showing the observed and expected tables, the test statistic, and the \(\scriptstyle p\)-value.
\item
...
\textbf{\textcolor{blue}{Key Concept:}}
\textbackslash textcolor\{blue\}\{\{In a chi-square goodness-of-fit test with \(\scriptstyle n\) observations:\}
...
\begin{align}
y_i &= \pi_i + \epsilon_i \\ \notag
    &= \beta_0 + \beta_1 x_i + \epsilon_i \quad \text{for } i = 1, 2, 3, \dots, n 
\tag{6.7}
\end{align}
...
\begin{align}
y_i &= \pi_i + \epsilon_i \\ \notag
    &= \frac{e^{\beta_0 + \beta_1 x_i}}{1 + e^{\beta_0 + \beta_1 x_i}} + \epsilon_i \quad \text{for } i = 1,2,3,\dots,n 
    \tag{6.8}
\end{align}
...
\textbf{\textcolor{blue}{Key Concept:}} \textbackslash textcolor\{blue\}\{When a 95\% Wald confidence interval for the odds ratio does not contain 1, we reject the null hypothesis \(\scriptstyle H_0: \ \beta_1 \ = \ 0\) (using an alpha-level of 0.05) and conclude that the odds of success do depend on the explanatory variable \(\scriptstyle x_i\). If the interval does contain 1, we fail to reject \(\scriptstyle H_0:\  \beta_1 \ =\  0\).\}
