...
4. Solve Equation (6.5) for $\scriptstyle \pi_i$ to show that Equation (6.6) is true.  

5. Use Equation (6.6) to create six graphs. In each graph, plot the explanatory variable ($\scriptstyle x$) versus the expected probability of success ($\scriptstyle \pi$) using $\scriptstyle \beta_0$ = $-$10 and $-$5 and $\scriptstyle \beta_1$ = 0.5, 1, and 1.5. Repeat the process for $\scriptstyle \beta_0$ = 10 and 5 and $\scriptstyle \beta_1$ = -0.5, $-$1, and $-$1.5.  
   a. Do not submit the graphs, but explain the impact of changing $\scriptstyle \beta_0$ and $\scriptstyle \beta_1$.  
   b. For all of these graphs, what value of $\scriptstyle \pi$ appears to have the steepest slope?  

6. Use statistical software to calculate the maximum likelihood estimates of $\scriptstyle \beta_0$ and $\scriptstyle \beta_1$. Compare the maximum likelihood estimates to the least squares estimates in Question 3.
...
15. Create a logistic regression model using Radius and Concave as explanatory variables to estimate the probability that a mass is malignant.  
   a. Using Radius as the first explanatory variable, $\scriptstyle x_1$, and Concave as the second explanatory variable, $\scriptstyle x_2$, submit the logistic regression model. In other words, find the coefficients for the model
...
a. What are the maximum likelihood estimates $\scriptstyle b_0$ and $\scriptstyle b_1$?  
b. Substitute $\scriptstyle b_0$ and $\scriptstyle b_1$ into Equation (6.9) to estimate the probability of malignancy when the radius is 4.5.  
c. Use Wald’s test and the $\scriptstyle G$-statistic to determine if this model provides evidence that the probability of malignancy is related to cell radius.
...
```
 ...
 \end{align}
   ...
   (d) Interpret the results and clearly state how your conclusions are impacted by random sampling and random allocation to treatments in the original study design. Remember that a small $\scriptstyle p$-value provides evidence that the model is *not* a good fit.

30. Calculate the deviance chi‐square goodness‐of‐fit test by hand for the Cancercells data given in Table 6.3. Expected counts, degrees of freedom, and $\scriptstyle p$-values are found the same way as in the Pearson test. The only difference is in the calculation of the test statistic:
...
34. For binomial response data that follow the binomial distribution, the log-likelihood function for one observation ($\scriptstyle y_1$) is  
