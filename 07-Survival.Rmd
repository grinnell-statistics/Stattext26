...
6. Consider the chocolate chip melting time data in Table 9.2. What is $\scriptstyle m$? List $\scriptstyle t_1$ through $\scriptstyle t_m$ for the chip melting times.  
...
Now if $\scriptstyle \hat p_i$ is the probability of an individual experiencing the event in the `i`th time interval, given that the individual has not experienced the event in the previous time intervals, then 1 - $\scriptstyle \hat p_i$  [[[[[]]]]] i is the probability of *not* experiencing the event (i.e., *surviving*) through the `i`th time interval, given that the individual has not experienced the event prior to the `i`th time interval.
...
9. What is the value of $\scriptstyle \hat p_1$? Interpret this value. 
...
11. Calculate the remaining estimated conditional probabilities $\scriptstyle \hat p_3$ and $\scriptstyle \hat p_4$. Place these values in the appropriate cells in Table 9.3 and interpret the values.  
12. Calculate the remaining estimated conditional probabilities 1 - $\scriptstyle \hat p_3$ and 1 - $\scriptstyle \hat p_4$. Place these values in the appropriate cells in Table 9.3 and interpret the values.
...
14. What is the estimate for $\scriptstyle S(45)$ in Table 9.3? That is, what proportion of chips in the sample has not melted after 45 seconds?  
...
Once the survival probabilities have been estimated, a graph of the Kaplan–Meier curve can be constructed to display the relationship between time and the estimated probability of surviving. The Kaplan–Meier curve is an approximation to the true survival curve, which is a graphical representation of `S(t)`. The values of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ are plotted against the complete event times $\scriptstyle t_1, \ t_2, \ \dots, \ t_m$. Figure 9.1 shows the Kaplan–Meier curve for the chocolate chip melting times displayed in Table 9.2.

Notice that the value of $\scriptstyle \hat S(30)_{\mathrm{KM}}$ remains the same across each time interval. From Table 9.3, we see that, for example, $\scriptstyle \hat S(30)_{\mathrm{KM}} = 5/7$; i.e., the proportion of chips in the sample that have not melted after 30 seconds is 5/7, or 71%. This value remains constant for time `t` in the interval [30,45). From Figure 9.1 we can observe that the Kaplan–Meier curve is a decreasing series of steps, with drops occurring at each complete event time $\scriptstyle t_i$. This type of plot is called a *step function* because it looks like a series of steps. The height of each step corresponds to the value of $\scriptstyle \hat S(t_i)_{\mathrm{KM}}$ for `t` inside [$\scriptstyle t_i$, $\scriptstyle t_{i+1}$), where $\scriptstyle i = 0,\ \dots,\ m-1$ (with the convention that $t_0 = 0$).
...
18. Compare the values of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ in Table 9.3 to those in Figure 9.1. How would the Kaplan–Meier curve in Figure 9.1 change if the largest observed melting time were not censored?  
...
Equations \ref{9.3} and \ref{9.4} can be thought of as a weighted average of the time-to-event data, where the Kaplan–Meier curve $\scriptstyle \hat S(t_i)_{\mathrm{KM}}$ acts as weights (probabilities) for the time-to-event intervals. Each term in the summation of Equations \ref{9.3} and \ref{9.4} represents the area of a rectangle formed by the width of the `i`th time interval $\scriptstyle t_{i+1} \ -\  t_i$ and the height of the Kaplan–Meier curve $\scriptstyle \hat S(t_i)_{\mathrm{KM}}$. For example, the width of the left-most rectangle in Figure 9.3 outlined by the vertical dotted line is 25 seconds, and the height of the rectangle is 1. So the first term in the sum of Equation \ref{9.3} is $\scriptstyle \hat S(t_0)_{\mathrm{KM}}(25 - 0) = (1)(25) = 25$.
...
23. For the chocolate chip melting time data in Table 9.2, calculate the estimated median survival time$\scriptstyle hat t_{(50)}$. Since right-censored data are typically right skewed, the median survival time is usually preferred to the mean.  
...
>**MATHEMATICAL NOTE** Based on the definition of the estimated percentile in Equation \ref{9.5}, it is possible that particular estimated percentiles do not exist. The $p$th percentile will not exist (and your software may return NA) if there does not exist a complete time $\scriptstyle t_p$ such that $\scriptstyle \hat S(t_p)_{\mathrm{KM}} \ \le\  1 \ -\  \tfrac{p}{100}$. For example, in the MeltingChipsJS data set, more than 25% of the milk chocolate chips had not melted by the end of the study (75 seconds). Thus, there is no estimate of the 75th percentile and the IQR cannot be calculated.
...
32. Refer to the entries in Table 9.5. Show that the expected number of melted milk chocolate chips in the first interval is 7/16 = 0.4375.

33. Refer to the entries in Table 9.5. Show that the variance in the first interval is 0.246.
...
where $\scriptstyle \sum_{i=1}^m V_{1i}$ is the variance of the total number of event occurrences over the $\scriptstyle m$ complete event times. Basically, Equation \ref{9.11} is nothing more than a `z`-score; i.e., it tells us how many standard deviations the total observed number of event occurrences is above or below its mean.
...
34. Calculate the missing quantities in Table 9.5. Note that $\scriptstyle E_{19}$, $\scriptstyle E_{1,10}$, $\scriptstyle V_{19}$, and $\scriptstyle V_{1,10}$ are all equal to 0. This is because all the milk chocolate chips have melted by 62 seconds.  
...
## \textcolor{blue}{The Cumulative Hazard}      
...
48. Use Equation \ref{9.17} and the quantities in Table 9.6 to compute $\scriptstyle \widehat H(t)_{\mathrm{NA}}$ for the chip melting times from Table 9.2 by hand.  
