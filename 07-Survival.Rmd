...
## \textcolor{blue}{Investigation: How Long Does It Take for Chocolate Chips to Melt?}
...
### \textcolor{blue}{Practice:} Melting Chocolate Chips {-}
...
>**NOTE**  If your class does not perform the chip melting activity, then you can use the data set MeltingChipsJS supplied by the authors to complete the activities and extended activities. Be certain to verify which data set (MeltingChips or MeltingChipsJS) your instructor would like you to use in the following activities.^[This data file contains the results of the chip melting activity administered by the second author of this textbook to his introductory survival analysis course. The maximum time allowed was 75 seconds]


## \textcolor{blue}{Overview of Survival Analysis Studies and Data}
...
## \textcolor{blue}{The Survival Function}
...
```{r tab91, echo=FALSE, results='asis'}
df9.1 <- rbind(
...
kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Hypothetical chocolate chip melting times for a sample of 7 students.",
  col.names = rep("", 7),
  align = c("l", rep("c", 7)),
  row.names = TRUE,
  escape = FALSE
...
When all observations are complete, we can generalize this calculation to any time `t` using an estimator of `S(t)` called the empirical survival function, denoted by $\scriptstyle \hat S(t)_{E}$ and given by
...
```{r tab92, echo=FALSE, results='asis'}
df9.2 <- rbind(
  Student = as.character(1:7),
...
kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Hypothetical chocolate chip melting times for a sample of 7 students, with incomplete times for students 1, 3, and 7.",
  col.names = rep("", 7),
  align = c("l", rep("c", 7)),
  row.names = TRUE,
  escape = FALSE
...
### \textcolor{blue}{Practice:} Empirical Survival Function {-}

4. Use Equation 7.1 and the 7 milk chocolate melting times in Table 7.1 to compute $\hat S(25)_{E}$, $\hat S(30)_{E}$, $\hat S(40)_{E}$, and $\hat S(60)_{E}$.  

5. With the melting times provided in Table 7.2, use the following two approaches to calculate the estimated probability that it takes more than 45 seconds for a chocolate chip to melt, based on the empirical survival function $\hat S(45)_{E}$:  
 (a) Treat all the censored times as complete (actual observed) times, and use Equation 7.1 to calculate $\hat S(45)_{E}$.  
 (b) Eliminate all censored observations, and then use Equation 7.1 and the remaining complete observations to calculate $\hat S(45)_{E}$.  
...
* The next interval begins with the complete time $\scriptstyle t_i$ and ends just prior to the next complete event time $\scriptstyle t{i+1}$. Time intervals of the form [$\scriptstyle t_i$, $\scriptstyle t_{i+1}$) are created for $\scriptstyle i = 1,
 2,\ \dots,\ m-1$.
...
### \textcolor{blue}{Practice:} Time Intervals for the Chip Melting Times {-}

6. Consider the chocolate chip melting time data in Table 7.2. What is $\scriptstyle m$? List $\scriptstyle t_1$ through $\scriptstyle t_m$ for the chip melting times.  
...
8. Determine $\scriptstyle d_i$, the number of melted chips in each interval, and $\scriptstyle n_i$, the number of chips at risk of melting in each interval (all chips with complete or censored times that have not yet occurred), for `i = 0, 1, 2, 3, 4`.

```{r tab93, echo=FALSE, results='asis'}
df9.3 <- data.frame(
...
kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Counts and estimated probabilities of melting for melting times.",
  align = c("r", "l", rep("c", 6)),
  escape = FALSE
) %>%
  kable_styling(latex_options = "hold_position") %>%
...
After the time intervals have been appropriately defined, we estimate $\scriptstyle \hat p_i$, the conditional probability of experiencing the event in the `i`th time interval, given that the event has not occurred by the start of the interval. That is, we compute
...
where $\scriptstyle d_i$ is the number of subjects who experienced the target event in interval `i` and $\scriptstyle n_i$ is the total number of subjects (with complete and censored times) who are eligible (at risk) to experience the target event at the *beginning* of the `i`th time interval.

Now if $\scriptstyle \hat p_i$ is the probability of an individual experiencing the event in the `i`th time interval, given that the individual has not experienced the event in the previous time intervals, then 1 - $\scriptstyle \hat p_i$ is the probability of *not* experiencing the event (i.e., *surviving*) through the `i`th time interval, given that the individual has not experienced the event prior to the `i`th time interval.

For example, the estimate of the conditional probability that a chip will *not* melt between the `25`th second and the `30`th second, given that it has remained unmelted through the `25`th second, is given by
...
That is, $\scriptstyle 6/7$, or about 86%, of the chips that have not melted just prior to the 25th second will remain unmelted (survive) between the `25`th and the `30`th second.

### \textcolor{blue}{Practice:} Estimated Conditional Melting Probabilities {-}

Use the chocolate chip data in Table 7.2 to answer the following questions:

9. What is the value of $\scriptstyle \hat p_0$? Interpret this value. 

10. $\scriptstyle \hat p_1$ is the estimate of the conditional probability that a chip will melt between the 25th second and the 30th second, given that it has remained unmelted through the 25th second. Show that about 14% of the chips that have not melted just prior to the 25th second will melt between the 25th and the 30th second.

11. Calculate the remaining estimated conditional probabilities $\scriptstyle \hat p_3$ and $\scriptstyle \hat p_4$. Place these values in the appropriate cells in Table 7.3 and interpret the values.  
12. Calculate the remaining estimated conditional probabilities 1 - $\scriptstyle \hat p_3$ and 1 - $\scriptstyle \hat p_4$. Place these values in the appropriate cells in Table 7.3 and interpret the values.
...
### \textcolor{blue}{Practice:} Kaplan–Meier Estimates {-}

Refer to the entries in Table 7.3 to answer the following questions.  

13. Use the remaining chocolate chip melting times to complete Table 7.3.  

14. What is the estimate for $\scriptstyle S(45)$ in Table 7.3? That is, what proportion of chips in the sample has not melted after 45 seconds?  

15. Use the entries in Table 7.3 to estimate the proportion of chips that have melted by 35 seconds.  

16. Use the entries in Table 7.3 to estimate the proportion of chips that have not melted after 50 seconds.  

17. Assume that no censoring is present in the melting times (see the entries in Table 7.1). Estimate `S(25)`, `S(30)`, `S(45)`, and `S(55)` using both the empirical survival function and the Kaplan–Meier estimator, and compare your answers. What do your answers suggest about the Kaplan–Meier estimator when no censoring is present?

### Graphing the Kaplan–Meier Curve {-}

Once the survival probabilities have been estimated, a graph of the Kaplan–Meier curve can be constructed to display the relationship between time and the estimated probability of surviving. The Kaplan–Meier curve is an approximation to the true survival curve, which is a graphical representation of `S(t)`. The values of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ are plotted against the complete event times $\scriptstyle t_1, \ t_2, \ \dots, \ t_m$. Figure 7.1 shows the Kaplan–Meier curve for the chocolate chip melting times displayed in Table 7.2.

Notice that the value of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ remains the same across each time interval. From Table 7.3, we see that, for example, $\scriptstyle \hat S(30)_{\mathrm{KM}} = 5/7$; i.e., the proportion of chips in the sample that have not melted after 30 seconds is 5/7, or 71%. This value remains constant for time `t` in the interval [30,45). From Figure 7.1 we can observe that the Kaplan–Meier curve is a decreasing series of steps, with drops occurring at each complete event time $\scriptstyle t_i$. This type of plot is called a *step function* because it looks like a series of steps. The height of each step corresponds to the value of $\scriptstyle \hat S(t_i)_{\mathrm{KM}}$ for `t` inside [$\scriptstyle t_i$, $\scriptstyle t_{i+1}$), where $\scriptstyle i = 0,\ \dots,\ m-1$ (with the convention that $t_0 = 0$).
...
### \textcolor{blue}{Practice:} Kaplan–Meier Curves {-}
18. Compare the values of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ in Table 7.3 to those in Figure 7.1. How would the Kaplan–Meier curve in Figure 7.1 change if the largest observed melting time were not censored?  
...
In the class data, you observed two possible chip melting *experiences* over time, corresponding to the milk and white chocolate chips. Figure 7.2 presents four additional Kaplan–Meier curves corresponding to different chip melting experiences.
...
### \textcolor{blue}{Practice:} Chip Melting Experiences {-}

20. Discuss the fourth chip melting scenario, illustrated by the Kaplan–Meier curve in Part (d) of Figure 7.2.

## \textcolor{blue}{Descriptive Statistics for Survival Data}
...
Equations 7.3 and 7.4 can be thought of as a weighted average of the time-to-event data, where the Kaplan–Meier curve $\scriptstyle \hat S(t_i)_{\mathrm{KM}}$ acts as weights (probabilities) for the time-to-event intervals. Each term in the summation of Equations 7.3 and 7.4 represents the area of a rectangle formed by the width of the `i`th time interval $\scriptstyle t_{i+1} \ -\  t_i$ and the height of the Kaplan–Meier curve $\scriptstyle \hat S(t_i)_{\mathrm{KM}}$. For example, the width of the left-most rectangle in Figure 7.3 outlined by the vertical dotted line is 25 seconds, and the height of the rectangle is 1. So the first term in the sum of Equation 7.3 is $\scriptstyle \hat S(t_0)_{\mathrm{KM}}(25 - 0) = (1)(25) = 25$.
...
### \textcolor{blue}{Practice:} Estimated Mean Chip Melting Time {-}

21. Examine Figure 7.3. Visually estimate the mean survival time (i.e., the estimated average time taken for the chocolate chips in the sample to melt) by computing a rough approximation to the area under the Kaplan–Meier curve.  

22. For the sample of chocolate chip melting times in Table 7.2, which equation, 7.3 or 7.4, is appropriate for estimating the mean survival time of the chips? Based on your answer, calculate the estimated mean using the quantities in Table 7.3.
...
The `p`th percentile of the distribution for the survival time random variable `T` is defined to be the time by which `p`% of the subjects in the population have experienced the target event. The estimate of the `p`th percentile, denoted by $\scriptstyle \hat t_{(p)}$, is defined to be the smallest *complete* event time in the sample such that at least $p\%$ of the subjects in the sample have experienced the event of interest before $\scriptstyle \hat t_{(p)}$, and no more than (100 - `p`)% of the subjects in the sample experience the event after $\scriptstyle \hat t_{(p)}$. Depending on the number of distinct complete event times, the value of $\scriptstyle \hat t_{(p)}$ can be found either by inspecting the Kaplan–Meier curve or by finding the solution to the following equation:
...
### \textcolor{blue}{Practice:} Estimated Median Chip Melting Time {-}

23. For the chocolate chip melting time data in Table 7.2, calculate the estimated median survival time$\scriptstyle hat t_{(50)}$. Since right-censored data are typically right skewed, the median survival time is usually preferred to the mean.  

24. Refer back to the Kaplan–Meier curve displayed in Figure 7.1. The mean and median survival times for the sample of chip melting times are provided in the “Table of Statistics” box on the graph. The interquartile range (IQR) is also provided. Recall that the IQR is the third quartile (75th percentile) minus the first quartile (25th percentile). Verify by hand that the IQR for the sample of times is 25 seconds.  
...
>**MATHEMATICAL NOTE** Based on the definition of the estimated percentile in Equation 7.5, it is possible that particular estimated percentiles do not exist. The $p$th percentile will not exist (and your software may return NA) if there does not exist a complete time $\scriptstyle t_p$ such that $\scriptstyle \hat S(t_p)_{\mathrm{KM}} \ \le\  1 \ -\  \tfrac{p}{100}$. For example, in the MeltingChipsJS data set, more than 25% of the milk chocolate chips had not melted by the end of the study (75 seconds). Thus, there is no estimate of the 75th percentile and the IQR cannot be calculated.


## \textcolor{blue}{Confidence Intervals for Survival Probabilities}
...
We know that the sampling distribution of $\scriptstyle \bar x$ is approximately normal for large sample sizes. A similar result can be stated concerning the sampling distribution of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ for fixed `t`. Some advanced theory tells us that, for larger sample sizes, the sampling distribution of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ at a fixed time `t` is approximately normal. This fact allows us to use critical values from the standard normal distribution.

Using the critical value from a standard normal distribution and the standard error of $\scriptstyle \hat S(t)_{\mathrm{KM}}$, we can put together the confidence interval for `S(t)` at a fixed time `t`. The usual procedure is to construct a confidence interval for `S(t)` at all the complete event times $\scriptstyle t_1,\ \dots,\ t_m$.

The 100(1-$\scriptstyle \alpha$)% confidence interval for `S(t)` at a fixed time `t` is given by
...
where $\scriptstyle Z_{\alpha/2}$ is the critical value from the standard normal distribution with $\scriptstyle \alpha/2$ area under the curve to its right (i.e., corresponding to confidence level 100(1 - $\scriptstyle\alpha$)%) and $\scriptstyle \mathrm{se}\bigl(\hat S(t)_{\mathrm{KM}}\bigr)$ is the standard error of the Kaplan–Meier estimator at time `t`, discussed in the following section.

### The Standard Error of the Kaplan–Meier Estimator {-}

The variability in the values of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ at a fixed time `t` in each interval [$\scriptstyle t_{i-1}$, $\scriptstyle t_i$) is measured by the estimated **variance of the Kaplan–Meier estimator**, denoted $\scriptstyle\widehat{\mathrm{Var}}\bigl(\hat S(t)_{\mathrm{KM}}\bigr)$ and calculated using the expression
...
The **standard error of the Kaplan–Meier estimator** at time `t`, denoted by $\scriptstyle \mathrm{se}\bigl(\hat S(t)_{\mathrm{KM}}\bigr)$, is the square root of $\scriptstyle \widehat{\mathrm{Var}}\bigl(\hat S(t)_{\mathrm{KM}}\bigr)$:
...
Then the standard error of $\scriptstyle \hat S(25)_{\mathrm{KM}}$ at time `t`=25 is $\scriptstyle \mathrm{se}\bigl(\hat S(25)_{\mathrm{KM}}\bigr) = \sqrt{0.0175}$ = 0.1323. 

With the computed standard error, we can use Formula 7.7 to construct a confidence interval for a survival probability. For example, the 95% confidence interval for `S(25)`—the probability that a randomly selected milk chocolate chip takes longer than 25 seconds to melt—is calculated as
...
### \textcolor{blue}{Practice:} Standard Errors and Confidence Intervals for Survival Probabilities {-}

Use the estimated survival probabilities and the entries in Table 7.3 to answer Questions 26 through 28, and use `MeltingChips` or `MeltingChipsJS` to answer Questions 29 through 31.  

26. Provide a brief explanation of why the estimated variance of $\scriptstyle \hat S(0)_{\mathrm{KM}}$, and hence the standard error of $\scriptstyle \hat S(0)_{\mathrm{KM}}$, is equal to 0.  

27. Find the remaining standard errors of $\scriptstyle \hat S(t)_{\mathrm{KM}}$ at times `t` = 30, `t` = 45, and `t` = 55 for the chocolate chip melting time data.  

28. Using your above answers and the appropriate entries in Table 7.3, construct 95% confidence intervals for the survival probabilities `S(t)` at each of the remaining complete melting times, and interpret their values.  
...
>**NOTE** The confidence interval for `S(t)` is valid for a single fixed time at which the inference is to be made. For this reason, confidence intervals are sometimes referred to as *pointwise intervals*. Each interval constructed using Formula 7.7 is relevant only for `t` within the `i`th interval. Therefore, it is not correct to state that we are 95% confident that the entire true survival curve `S(t)` falls between the confidence bands in Figure 9.5. If we want a confidence band for an entire survival function `S(t)` within which we can guarantee with a specified level of confidence that the entire curve falls, alternative confidence bands need to be computed.  


## \textcolor{blue}{Comparing Survival Functions}
...
experiencing the event. The alternative hypothesis is that the survival experiences are not identical for at least one value of `t` (i.e., $\scriptstyle H_a:\;S_1(t)\ \neq \ S_2(t)$ for some `t`).
...
kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Table 7.4 Hypothetical melting times of the milk and white chocolate chips.",
...
cat(
  kable(
    ...
    format = "latex",
    booktabs = TRUE,
    caption = "Table 7.5 Selected quantities for the combined groups of chocolate chips.",
...
>**MATHEMATICAL NOTE**  Equation 7.10 is based on the assumption that the number of occurrences for group 1 at time $t_i$ follows a hypergeometric distribution.$^6$
...
### \textcolor{blue}{Practice:} Calculating the Expected Number and Variance of Melted Chips {-}
  

32. Refer to the entries in Table 7.5. Show that the expected number of melted milk chocolate chips in the first interval is 7/16 = 0.4375.

33. Refer to the entries in Table 7.5. Show that the variance in the first interval is 0.246.
...
where $\scriptstyle \sum_{i=1}^m V_{1i}$ is the variance of the total number of event occurrences over the $\scriptstyle m$ complete event times. Basically, Equation 7.11 is nothing more than a `z`-score; i.e., it tells us how many standard deviations the total observed number of event occurrences is above or below its mean.
...
### \textcolor{blue}{Practice:} Log-Rank Test Statistic {-}

34. Calculate the missing quantities in Table 7.5. Note that $\scriptstyle E_{19}$, $\scriptstyle E_{1,10}$, $\scriptstyle V_{19}$, and $\scriptstyle V_{1,10}$ are all equal to 0. This is because all the milk chocolate chips have melted by 62 seconds.  
35. Use Equation 7.12 and the entries in Table 7.5 to verify that the value of the log-rank test statistic is 1.007. Note that your answer may vary slightly because of rounding.
...
### \textcolor{blue}{Practice:} Log-Rank Test for Chip Melting Activity {-}
...
## \textcolor{blue}{What Can We Conclude About Melting Chocolate Chips?}
...
## \textcolor{blue}{The Hazard Function}
...
Equation 7.13 may look something like the formal definition of the derivative of some function (although it probably isn’t clear what that function is). It turns out that `h(t)` is related to the derivative of a function of the (population) survival function `S(t)` by the expression
...
>**NOTE** The R statistical package is suggested for calculating and graphing the estimated hazard and estimated cumulative hazard functions discussed in this section and Section 7.9.  
...
Recall that the estimated conditional probability of experiencing the target event in the `i`th interval $\scriptstyle [t_i,\,t_{i+1})$ is given by
...
for $\scriptstyle i=0,1,\dots,m-1$. Recall that $\scriptstyle \hat p_i$ represents the proportion of remaining subjects in the sample at the beginning of the `i`th interval who experience the target event in the time interval $\scriptstyle [t_i,\,t_{i+1})$.
...
yields the *average* conditional probability of event occurrence per unit of time within $\scriptstyle [t_i,\,t_{i+1})$. This quantity is the **estimated hazard rate** or **estimated hazard function** for $\scriptstyle t_i \le t < t_{i+1}$, denoted by $\scriptstyle \hat h(t)_{\mathrm{KM}}$, and measures the estimated conditional probability of experiencing the event *per* unit of time in $\scriptstyle [t_i,\,t_{i+1})$. When no interval of time is specified, $\scriptstyle \hat h(t)_{\mathrm{KM}}$ will be used to refer to the estimated hazard function for any `t` within the entire observed range of time.
...
### \textcolor{blue}{Practice:} Chip Melting Hazard Rates {-}
37. Compute the missing entries in Table 7.6.  
...
40. Do the values of $\scriptstyle \hat h(t)_{\mathrm{KM}}$ suggest that the estimated hazard function is a strictly increasing or strictly decreasing function, or do they suggest that the function can increase and decrease over time?

We can plot the estimated hazard rates versus time to view the estimated **hazard curve** for the sample of survival times. The Kaplan–Meier curve and estimated hazard function for the chocolate chip melting times are shown in Figure 7.9. Note that, because of the small sample size, the plot is extremely rough and the pattern is somewhat erratic. This curve is an approximation to the corresponding true hazard function for the population of all chocolate chip melting times. From Figure 7.9 we observe that the lowest risk of melting occurs between 30 and 45 seconds (among chips that have remain unmelted up through 30 seconds). The highest risk of melting occurs between the 45th and the 55th second. Note that the estimated hazard rate is not defined before the first complete event time and after the last complete time.
...
### \textcolor{blue}{Practice:} Estimated Hazard Rates {-}
...
### \textcolor{blue}{Practice:} Age at First Alcoholic Drink {-}
Data set: `Firstdrink`  
...
### \textcolor{blue}{Practice:} College Graduation {-}
Data set: `Graduate`
...
## \textcolor{blue}{The Cumulative Hazard Function}      
...
Similar to the survival function and hazard function, we can define a cumulative hazard function for a population and a sample. We will denote the population cumulative hazard function by `H(t)`, and the estimator of the cumulative hazard function by $\scriptstyle \widehat H(t)_{\mathrm{KM}}$. The definition of `H(t)` is given by
...
Figure 7.13 presents the hazard functions originally shown in Figure 7.8 and their corresponding cumulative hazard functions. We can see that if the hazard rate is constant over time [Part (a)], then the cumulative hazard function [Part (c)] will increase at a _constant_ rate—that is, there is a perfect positive linear relationship between values of t and values of H(t). If the hazard rate is increasing linearly over time [Part (b)], then the rate of change in H(t) is increasing [Part (d)].
...
where $\scriptstyle \hat h(t)_{\mathrm{KM}}$ is the estimated hazard rate for $\scriptstyle t_i  \ \le \ t  \ <  \ t_{i+1}$ and $\scriptstyle t_{i+1}  \ - \  t_i$ is the width of the `i`th interval.  Then the Nelson–Aalen estimator of `H(t)`, denoted $\scriptstyle \widehat H(t)_{\mathrm{NA}}$, is simply the sum of these total estimated hazard quantities up to a particular time `t`, given by
...
### \textcolor{blue}{Practice:} Estimated Cumulative Hazard Function {-}

48. Use Equation 7.17 and the quantities in Table 7.6 to compute $\scriptstyle \widehat H(t)_{\mathrm{NA}}$ for the chip melting times from Table 7.2 by hand.  

As discussed earlier in connection with the population cumulative hazard function `H(t)`, we can also examine how the rate of change in the estimated cumulative hazard, $\scriptstyle \widehat H(t)_{\mathrm{NA}}$, changes over time, to investigate changes in the estimated hazard function over time.

- If the _rate of change_ in $\scriptstyle \widehat H(t)_{\mathrm{NA}}$ is _increasing_ (over an interval of time), then $\scriptstyle \hat h(t)_{\mathrm{KM}}$ is increasing (over the same interval of time).  
- If the _rate of change_ in $\scriptstyle \widehat H(t)_{\mathrm{NA}}$ is _decreasing_, then $\scriptstyle \hat h(t)_{\mathrm{KM}}$ is decreasing.  
- If the _rate of change_ in $\scriptstyle \widehat H(t)_{\mathrm{NA}}$ is _constant_ (and greater than 0), then $\scriptstyle \hat h(t)_{\mathrm{KM}}$ is constant (and greater than 0).  
- If the _rate of change_ in $\scriptstyle \widehat H(t)_{\mathrm{NA}}$ is 0, then $\scriptstyle \hat h(t)_{\mathrm{KM}}$ is 0.  
...
## \textcolor{blue}{Additional Types of Incomplete Data}
...
### \textcolor{blue}{Practice:} Left Censoring and Interval Censoring {-}
...
To provide a range of possible values for the true survival probabilities, confidence intervals based on the Kaplan–Meier estimates can be constructed for `S(t)` at fixed points in time.  The 100(1 $\scriptstyle -\  \alpha)$% confidence interval for the survival probability `S(t)` at fixed time `t` is given by
\begin{align}
\hat S(t)_{\mathrm{KM}} \pm Z_{\alpha/2}\,\mathrm{se}\bigl(\hat S(t)_{\mathrm{KM}}\bigr),
\end{align}
where
...
cat(
  kable(
    ...
    format = "latex",
    booktabs = TRUE,
    caption = "Table 7.7 Quantities associated with patients with AML who received maintenance chemotherapy.",
